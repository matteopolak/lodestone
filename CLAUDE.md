...
- **`-j` bounds rustc, not test binaries** — single test binaries here measure 4.8–5.2 GB RSS against
  ...
  compiles**, and give each concurrent Cargo run its own literal `--target-dir` and bounded `-j` value as
  documented in [`docs/build-caching.md`](./docs/build-caching.md). **Never background a Cargo run and
  stop the agent that owns it.**
