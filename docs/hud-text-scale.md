...
* **Chat has the same double-apply.** `chat_pose_scale` (`crates/lodestone-shell/src/hud.rs`) is `scale * opts.scale`, which is 2.0 at the default `chatScale` of 1.0, where vanilla's chat pose scale is `chatScale` **alone** (`ChatComponent.getScale`). So the chat box is 2× vanilla too. Not part of the reported bug and not covered by the gate above — recorded here so the next reader does not rediscover it. Still true as of the scoreboard-sidebar audit below; deliberately not fixed there because the change ripples through `chat_line_h`, `chat_width_px`/`chat_height_px`, the input strip, and `suggestion_layout` (used outside `build_inner` for pointer hit-testing too) — a single-file, single-block fix like the sidebar's is not available here.
