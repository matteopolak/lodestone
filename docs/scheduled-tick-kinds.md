...
`StagedTick` carry the typed key. The connection feed carries that typed key
unchanged; only Anvil saving converts it to `chunk_nbt::SavedTick`, while
native snapshots retain the typed key.
...
`BlockTickFeed::request_scheduled_ticks`; fluid producers should call
`BlockTickFeed::request_fluid_scheduled_ticks` with a typed
`ScheduledTickKind`. Keep parameterized piston records
...
`lodestone-storage-schema`; the integrated server's block producers, fluid
implementation, and connection feed use typed keys. Anvil and focused fixture
adapters retain the remaining explicit string boundaries.
