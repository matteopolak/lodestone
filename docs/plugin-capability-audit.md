...
| permission nodes, wildcards, defaults, groups, delegation | **done** — `PermissionStore`, `PermissionRegistry`, `PermissionResolver`, `Permissions` resource | **gap** | partial — the five vanilla levels plus ops/whitelist/bans; node permissions live only in `lodestone-ecs`, reachable only through the sink |
