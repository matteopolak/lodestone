...
`WorldExportInput` names every chunk to export and supplies the native vertical extent, tick-conversion game time, region compression scheme, and timestamp. Its named builder validates the input and canonicalizes chunk coordinates, while the positional `new` function remains as a compatibility wrapper. Neither caller ordering nor the current clock can affect the resulting terrain files. The dedicated conversion command additionally offers an explicit `--all-terrain` selection: it snapshots the native store's recovered latest chunk-key index before building this same explicit input. Version-1 native keys have no dimension value, so that selection is horizontal terrain columns only.

`preflight_world_export` loads every named record and aggregates the existing per-chunk loss reports. `export_world_directory` repeats that load as its source snapshot, checks one `WorldExportAuthorization` against the full aggregate, and converts every chunk in memory before touching the destination filesystem. Use `snapshot_world_export` when an in-process caller needs the same point-in-time guarantee for a reviewed explicit selection: it holds the storage lock through every selected typed decode and owns those records for `preflight_native_world_export` and `export_native_world_snapshot`. The one existing loss is pending tick insertion order; the destination preserves list order but does not retain the native scheduler sequence.

`snapshot_native_world_export` is the all-native variant for an in-process reviewed operation. It captures `WorldStorage::native_chunk_records` under the storage lock, owns every decoded `NativeChunkRecord`, and rejects empty or incomplete native terrain before returning. `preflight_native_world_export` and `export_native_world_snapshot` then use that same owned data, so an incremental write after preflight cannot silently replace a reviewed chunk in the published Anvil directory. Both snapshot forms are point-in-time values, not live views.
...
Keep `WorldExportInput` explicit and use its named builder at new call sites; the compatibility `new` wrapper must continue to run the same validation and coordinate canonicalization. The `--all-terrain` caller obtains one copied native-key snapshot through `WorldStorage::native_chunk_coordinates` before constructing the input; do not turn an empty snapshot into a successful empty export. `snapshot_world_export` and `snapshot_native_world_export` are deliberately separate from key-only selection because they own decoded records; use the former for a reviewed subset and the latter for all committed terrain. Add any new lossy native field to the one-chunk exporter first, then ensure `WorldExportReport::unsupported_count` includes it through the per-chunk report. Do not add opaque payload copying here: source fields either have an existing typed Anvil conversion or are reported and discarded under authorization.
...
There are no environment variables or implicit defaults. Explicit callers choose chunk coordinates, `min_y`, `height`, tick `game_time`, `CompressionScheme`, and region timestamp in `WorldExportInput`; all-native callers pass the latter five values to `snapshot_native_world_export`. Both pass a destination path that does not exist. The staging directory is a deterministic same-parent sibling named `.<destination>.lodestone-export-staging`.
...
The module composes `world_storage::WorldStorage`, `anvil_export`, and `lodestone_anvil::region`. `bon` generates the required-field builder from the compatibility constructor. `anvil_export` owns chunk NBT conversion and loss semantics; the Anvil crate owns compression, region layout, and external sidecars.
