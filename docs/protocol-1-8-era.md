...
Protocol 47 now carries the basic inventory session through the shared host
boundary. `V47Adapter` encodes `ClientAction::ContainerClick` as the legacy
`window_click` frame, using the model's synchronisation number as the bounded
transaction field and leaving the legacy clicked-stack claim empty. The
authoritative server consumer decodes the window, slot, button, and mode into
`ServerBound::ContainerClicked`; its normal container correction then reaches
the adapter's existing `window_items`/`set_slot` events. This is deliberately
limited to family-level item stacks: the canonical model does not guess
pre-flattening damage variants, so the server's correction remains the source
of truth for those items.

The literal decoder control in `crates/versions/1.8/tests/server_protocol.rs`
and adapter encoding control in `crates/versions/1.8/tests/interaction.rs`
pin both ends of the frame. The hosted chest integration remains the live
proof that opening a chest, moving one item, receiving authoritative content,
and closing the window all reach the production path.
...
`crates/versions/1.8/tests/capture_join.rs` records a bounded clientbound
stream from the same real 1.8.9 oracle and replays the committed bytes through
`V47Adapter` in the ordinary test suite. The replay checks the server-selected
overworld/survival join, loaded columns, keep-alive handling, and the flat
world's bedrock/dirt/grass floor in canonical ids. It also feeds the existing
real 1.7.10 and 1.9.4 captures through protocol 47 and requires that neither
neighbor becomes a clean join. Re-record with:
...
The fixture retains every observed packet id, at most three bodies per id, and
at most two chunk bodies; its bytes are never generated by this adapter.
