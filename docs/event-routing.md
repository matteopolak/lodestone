...
component and the F3 entity line shows the reported `SD` value only after that packet arrives; it
stays distinct from the streamed-view radius that sizes the loading grid. `ItemCooldown` folds into
