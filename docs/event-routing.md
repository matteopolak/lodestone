...
**5 of 136** variants are currently `Route::NOWHERE`. Most of those are simply decoded ahead
...
The current terminal routes are `SimulationDistanceChanged`, `PlayerCombatEntered`,
`PlayerCombatEnded`, `MountScreenOpened`, and `ServerDataReceived`. `ItemCooldown` folds into
