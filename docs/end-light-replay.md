...
The End fallback also reproduces the initial light engine's storage admission: any admitted non-air section allocates a contiguous vertical corridor from the first in-world light section through the upper one-section apron. This preserves explicit empty block-light layers and full sky layers in empty sections below terrain; an all-air footprint still allocates no layers. Nether keeps its per-section adjacency rule because it has no sky layer.
