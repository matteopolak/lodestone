...
The privileged `version:broker` capability is a separate opt-in. A manifest that requests it must also declare `[version-lock]` with an exact `family`, `protocol`, and broker `abi`; the host compares all three against its configured `VersionBroker` before reading or compiling the module. The broker WIT import returns only copied descriptor and key/value records, and the default policy withholds it. Configure an embedding with `PluginHost::with_version_broker`; a missing source or any mismatch is a loud load error.
...
When adding a version-specific broker consumer, update the manifest's `[version-lock]` values and the host's configured descriptor together. Keep the WIT import by-value: extending it with ECS handles, world guards, sockets, or arbitrary callbacks would cross the plugin trust boundary.
