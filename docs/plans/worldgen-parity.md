...
(the original phased plan — its Phases 1–3 have since **landed**: #405, #295, #406/#427 all closed),
...
level/chunk/status/ChunkStatus.java:21-32`): `empty → structure_starts → structure_references →
biomes → noise → surface → carvers → features → initialize_light → light → spawn → full`, with the
`features` status fanning out into eleven decoration steps (`GenerationStep.java:8-18`) run per biome
...
| 6b | `VEGETAL_DECORATION` | **partial: composed 3×3, named gaps** | `vegetation_parity`: bit-exact at 4 fixtures modulo gaps; `KNOWN_VEGETATION_GAPS` (`crates/lodestone-server/src/worldgen_data.rs:275-336`) names them per biome: `multiface_growth` (glow lichen — in **every** biome's list), `fallen_tree`, fancy/giant trunks (jungle, dark oak, mangrove, cherry — #428; acacia closed in `04bfb57`), kelp/seagrass/coral/`sea_pickle` (oceans), bamboo, vines, huge mushrooms, `root_system`/`vegetation_patch` (lush caves), `random_boolean_selector`, two `simple_block`/`block_column` config variants. Plus two measured, mechanism-unknown savanna residuals (11 leaf-`distance` cells; 1 `short_grass` cell) |
...
| — | old-chunk blending (`Blender.of(region)`, `BelowZeroRetrogen`, `ChunkStatusTasks.java:82,94-96`) | **absent, deliberately out of scope** | only matters for worlds imported from older versions |
...
- One **author-independent end-to-end control**: generate a region on the vanilla `terrain.sh`
  oracle server, read the region file via `lodestone-anvil` (#437 lands the reader), and diff
...
### 4. The version seam (#407) — verdict on shape
...
trait. That trait already exists: `lodestone_server::ChunkSource` (`chunk.rs:434` — 
`OverworldChunkSource` is the 26.2 implementation). The selection idiom already exists too:
`lodestone-registry`'s `SERVER_FAMILIES` table (`crates/lodestone-registry/src/lib.rs:151-158`),
...
- Move `crates/lodestone-server/assets/worldgen/` + the `build.rs` embedding + `EmbeddedResolver`
  into `crates/protocol/v770`. This is #407's own prescription, and it survives the #361/#343
  check: `lodestone-data` holds the *canonical censuses* of the one internal version, but
  per-version **behavior** data (like v340's flattening table) lives in its family crate — worldgen
  JSON is behavior data the owner explicitly wants selectable. Note #407's pointer "mirror
  lodestone-v770's existing registry data mechanism" is stale — that mechanism moved to
  `lodestone-data` in #361; the thing to mirror is v340's in-family table pattern plus
...
`mobs.rs` in `lodestone-server` are **owned by other agents at the time of writing** — U8 waits.
...
| U7 3-D biomes | per-quart-cube climate sampling; `Climate.RTree` port (`Climate.java:248`) with brute-force (`:182`) as its own in-tree control | `biome.rs`, `BiomeOracle.java` | perf gate from U9 before landing |
| U8 seam (#407) | §4 as written | `worldgen_data.rs`, `assets/worldgen/` move, v770 additions, registry field | `integrated.rs`/`server.rs` owners; lib.rs broker |
| U9 benches (#85/#87) | `StageTimes` extended past its current 4 fields (shape/fluid_heightmap/surface/intern — it predates carve/ore/vegetation entirely); persisted split; RD 8/16/32 sweep with peak RSS via `lodestone-allocbench`'s `/usr/bin/time -l` pattern | `benches/generation.rs`, `column_timed`/`StageTimes` in overworld.rs (small brokered touch) | — |
| S1..Sn structures | placement grid first (deterministic, `/locate`-checkable), then jigsaw/template/terrain-adaptation as separate units | new `crates/lodestone-worldgen/src/structure/` (or sibling crate) | after U1–U7; unblocks #136 |
...
- The 3×3 drivers' 9× recompute is already amortized by `pre_ore_cache`/`post_ore_cache` (512-entry
  FIFO) for sweep-shaped access; the open cost item is release-profile confirmation plus #87's
  peak-RSS story (each cache entry is a ~200 KiB grid; two caches ≈ up to ~200 MiB worst case —
...
4. **Wire parity ≠ block parity**: served chunks carry empty heightmaps and `Missing` light
   (`server_protocol.rs:1058-1061`), and fluids lack the `level` property. Real gaps, tracked, not
...
- #407's "mirror v770's registry-data mechanism" predates #361 moving that mechanism to
  `lodestone-data` (§4).
- `worldgen-plan.md` §0/§3 describe carvers/aquifer/features as "not composed" — true at writing,
  closed since (#295, #406, #427). That doc is kept as the historical argument; this plan is the
  current state.
...
run.sh`); see #430 for the Apple-container migration state.
...
selection table), `lodestone-anvil` (#437, the author-independent control), issues #404 #407 #428
#85 #87 #136 #437.
