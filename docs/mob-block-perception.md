...
(`EatBlockGoal.IS_EDIBLE`/`EatBlockGoal.canUse`). A block id would drag a registry into
...
options originally proposed, and cheaper than both.

## The seven `Missing` rows: what the seam actually closes

The original report's headline is that *seven* unmodelled roster rows across two families
...
|---|---|---|
| sheep `EatBlockGoal` | two local cues (`EatBlockGoal.canUse`) | **yes** |
| skeleton `RestrictSunGoal` | **no block read at all** — `level.isBrightOutside()` plus an empty HEAD slot (`RestrictSunGoal.canUse`). Its *effect* is `GroundPathNavigation.setAvoidSun(true)`, i.e. a sky-light penalty in the path evaluator | no — a pathfinder feature |
| skeleton `FleeSunGoal` | `isBrightOutside`, `mob.isOnFire()`, `canSeeSky(pos)` (a column query, not a neighbour cue), and 10 random probes for a shaded position (`FleeSunGoal.getHidePos`) | no — needs a host-computed candidate position |
| zombie `ZombieAttackTurtleEggGoal` | `RemoveBlockGoal` over a 24-block spiral with vertical range 3, plus block-break progress and a destroy intent (`Zombie.registerGoals`, `RemoveBlockGoal.java`) | no — candidate position + destroy intent |
| rabbit `RaidGardenGoal` | `MoveToBlockGoal(0.7, 16)` spiral, the `#supports_crops` tag, and `CarrotBlock.AGE` — a block-state **property**, not a boolean cue (`Rabbit.RaidGardenGoal`) | no — candidate position + state properties |
| rabbit `ClimbOnTopOfPowderSnowGoal` | one local cue (block above is powder snow or has empty collision) **and** `mob.isInPowderSnow`/`wasInPowderSnow`, which no physics here sets (`ClimbOnTopOfPowderSnowGoal.canUse`) | cue yes, goal no — blocked on powder-snow physics |
...
* **Adding a mutation.** Add a variant to `EatenBlock` (or a sibling intent
  ...
  the block change (`EatBlockGoal.tick`), so the gamerule check belongs on
...
* **The gotcha that cost the most time here.** Every tick constant in
  ...
  `requiresUpdateEveryTick`, and this one does not (`Goal.adjustedTickDelay`/`Goal.reducedTickDelay`).
...
1. `crates/lodestone-server/src/mobs/world.rs` — `ChunkWorld` must implement
   `block_cues`, and `crates/lodestone-server/src/mobs/mod.rs`'s `MobSim::tick` must drain `take_new_eaten` and perform the
...
Sheep grazing additionally needs `Sheep.ate()`'s wool regrowth —
