...
| `entity_night_pixels::a_sky_lit_mob_is_darker_at_midnight_than_at_noon` | **the midnight magnitude at pixels, on the RED channel**: 0.278411, with a slack of 2 8-bit steps on the predicted midnight *byte*, rejecting 0.504652 (`SkyLightColor` dropped — vanilla's blue), 0.216817 (ambient dropped), 0.392 (retired ramp) and 1.000, each asserted rather than described | which curve terrain uses |
