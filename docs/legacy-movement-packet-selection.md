...
The v1-8 (Minecraft 1.8.9), v1-9 (1.12.2), and v1-14 (1.16.5) protocol adapters
...
v1-9 and v1-14 increment their `position_reminder` before that comparison and
...
v1-8 has the same `9e-4` position threshold but an older idle rule: it always
...
packet when it differs from that baseline (or, for v1-9/v1-14, when it changes
...
logic changes. Do not merge these into v26-2's tracker without first preserving
the version differences: v26-2 uses a `(2e-4)^2` threshold and additionally
tracks horizontal collision, while v1-8 must keep its every-tick idle packet.

## Configuration

There is no runtime configuration. Protocol choice determines the rule:
`PROTOCOL = 47`, `340`, or `754` (the v1-14 directory represents 1.16.5's
protocol 754).
