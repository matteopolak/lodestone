...
Add a built-in variant and both directions in `BlockEntityKind::from_name` and `BlockEntityKind::name` when a new built-in becomes part of the server's modeled set. `BlockEntity::type_id` follows automatically because it delegates through `kind`; no second string match should be added. Internal gameplay decisions should match `BlockEntity::kind`; use `type_id`, `name`, or `into_name` only at a format or protocol boundary. Never discard an `Extension` value while decoding an opaque or container record.
