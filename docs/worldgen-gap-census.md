...
| `world_preset/*.json` documents (7) + flat presets (9) | **partly reached (data)** | Updated 2026-08-14: `flat`/`flat_all_dimensions`/`single_biome_surface`'s own embedded `generator.settings`/`biome_source.biome` fields are now read directly (`worldgen_data::world_preset_flat_settings`, `world_preset_single_biome_default_biome`) — the first code in this crate to parse a `world_preset/*.json` document rather than treat it as an `EXPECTED_COUNTS` drift constant. `amplified`/`large_biomes`/`normal` and `debug_all_block_states` still do not: the first two go through the hand-written `WorldType` enum naming the `noise_settings` id directly (unchanged from the prior note), and the debug preset's document carries no settings to read (`{"type": "minecraft:debug"}`, fixed by definition) |
...
| superflat generator | **reached** (issue #519, `73de79fe`) | `lodestone_worldgen::flat::FlatLevelSource` ports `FlatLevelGeneratorSettings`/the layer-stack fill; `worldgen_data::FlatChunkSource`/`flat_chunk_source` wraps it as a real `ChunkSource`, built entirely from `ChunkColumn`'s public API. This row was stale in this document from `73de79fe` onward — that commit updated `worldgen_data.rs`'s own module doc but not this census; corrected here 2026-08-14, discovered by re-checking every #519 row against the tree rather than trusting the prior text |
| debug-world generator | **reached** (issue #519, 2026-08-14) | `lodestone_worldgen::debug::DebugLevelSource` ports `DebugLevelSource.java` (block-state grid at y=70, `minecraft:barrier` floor at y=60, `FixedBiomeSource(plains)`); `worldgen_data::DebugChunkSource`/`debug_chunk_source` wraps it. Verified against the real 32,366-state registry: `GRID_WIDTH`/`GRID_HEIGHT` both `180` (`ceil(sqrt(32366))`), and world `(17,17)` resolves to `minecraft:note_block[instrument=trumpet_exposed,note=8,powered=false]` — a real multi-property state, not a placeholder (`crates/lodestone-server/src/worldgen_data.rs`'s `single_biome_and_debug_world_selection` test module) |
| `FixedBiomeSource` | **reached** (issue #519, 2026-08-14) | Needed no new biome-source type: `OverworldGenerator::new`'s existing fixed-biome fallback (`dynamic_biome: None`, selected whenever `Resolver::biome_parameters` returns its empty default) already *is* `FixedBiomeSource` — it was built for degradation and never before deliberately selected. `worldgen_data::SingleBiomeResolver` is a resolver that withholds `biome_parameters` on purpose; `single_biome_generator(seed, biome)` builds the generator through it. Verified: `single_biome_chunk_source(4242, "minecraft:desert")` reports biome `minecraft:desert` at three sampled chunks where the default per-column overworld reports three different biomes (`snowy_plains`, `snowy_plains`, `plains`), none of them desert |
| `CheckerboardColumnBiomeSource` | **absent** | grep `CheckerboardColumnBiomeSource` → 0 hits. Not needed by any of the seven `world_preset` documents this crate bundles (checked 2026-08-14: none of `{normal,amplified,large_biomes,single_biome_surface,flat,flat_all_dimensions,debug_all_block_states}.json` reference it) — vanilla's own use is the "Customize World" debug/testing biome-source option, not a shipped preset. #519's own scope named it as a shared dependency of "single-biome and debug presets", which this re-check found to be about `debug_all_block_states`' *codec* only (`DebugLevelSource`'s constructor takes `FixedBiomeSource`, not checkerboard) — so nothing landed in this issue was actually blocked on it |
...
| `TheEndBiomeSource` / `FixedBiomeSource` / `CheckerboardColumnBiomeSource` | **partial, updated 2026-08-14** | `FixedBiomeSource` is now **reached** (see §1's row); `TheEndBiomeSource` and `CheckerboardColumnBiomeSource` are still **absent** — not touched by issue #519, and not needed for any bundled `world_preset` (see §1's `CheckerboardColumnBiomeSource` row) |
...
| 10 | **World presets: no player-facing way to pick one** | **re-verdicted 2026-08-14**: all 5 overworld generator types this crate can build now exist and are individually verified (`amplified`/`large_biomes`/superflat landed earlier in #519; single-biome and debug landed this pass — `FlatLevelSource`, `SingleBiomeResolver`, `DebugLevelSource`). **What remains is entirely `lodestone-shell/src/menu/create_world.rs`**: nothing outside `lodestone-worldgen`/`lodestone-server` constructs any of these five, so a player still cannot select any of them — the generator-side gap is closed, the UI gap is not | generators reached 5/5; UI selection still absent (§1) |
...
| world presets / alternate generators | **#519** | amplified/large_biomes/superflat/single-biome/debug all landed (2026-08-14); world-creation-screen wiring is the only item still open |
