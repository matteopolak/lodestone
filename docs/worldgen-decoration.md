...
Source-less compact fixtures model the same split with a sparse seeded baseline: the WG lane reads
those pre-decoration cells, while later writes remain visible only to the live overlay lanes.
...
overlay can replace the selected fossil's blocks without creating a second placement surface. The
processor-list and rule-test records cross a closed Serde boundary: unsupported discriminator
...
After each tree's trunk, foliage, decorators and roots are written, leaf-distance propagation uses
the tree's own inclusive bounding box. Its per-distance worklists retain the source set's explicit
hash-bucket order (16 buckets initially, 0.75 load factor, insertion-ordered collision chains),
while a separate filled set prevents a log from re-queuing itself. This is intentionally not a
visited-on-enqueue shortest-path BFS: a leaf can be queued at two distances, and the later stale
entry rewrites its state. The authenticated seed-42 Overworld target at chunk `(-8,-8)` is the
production control: it contains 393 dark-oak leaves and 157 logs, with local `(0,77,0)` and
`(0,78,0)` resolving to leaf distances 2 and 3 (global state ids 427 and 431). A focused four-leaf
control retains the later distance-3 rewrite; changing the queue to ordinary BFS fails that control
and changes the target's heightmaps.
