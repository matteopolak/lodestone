...
| Cancellation (`setCancelled`) | partial (native tier) | `ActionVetoes` (`crates/lodestone-ecs/src/veto.rs`) asks a per-verb predicate before the effect is computed, priority-keyed, first `Deny` wins; five of six declared verbs are asked in production (`BlockBreak`, `BlockPlace`, `EntityDamage`, `InventoryClick`, `PlayerMove`), with only `PlayerInteract` deferred | [#101](https://github.com/matteopolak/lodestone/issues/101) (design) |
| Cancellation of the concrete high-value verbs (break/place/damage/click/move) | done (native tier) | all five are wired through `ActionVetoes`; the broader interaction verb remains deferred | [#109](https://github.com/matteopolak/lodestone/issues/109) |
