...
| Cancellation (`setCancelled`) | done (native tier) | `ActionVetoes` (`crates/lodestone-ecs/src/veto.rs`) asks a per-verb predicate before the effect is computed, priority-keyed, first `Deny` wins; all six declared verbs are asked in production (`BlockBreak`, `BlockPlace`, `EntityDamage`, `InventoryClick`, `PlayerMove`, `PlayerInteract`) | [#101](https://github.com/matteopolak/lodestone/issues/101) (design) |
| Cancellation of the declared interaction verbs | done (native tier) | all six are wired through `ActionVetoes`; `PlayerInteract` asks once before entity, block, or air interaction effects and carries the chosen target in its context | — |
