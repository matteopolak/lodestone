...
If a supported candidate's nested feature makes no write, the scan continues to the next candidate. The failed nested attempt may have consumed draws, so continuing on the same source preserves the stream seen by later candidates and by the root passes. Candidate predicates retain their configured offsets, including the bundled check of the block directly below an elevated candidate.
...
Keep the nested-placement callback on the caller-owned random stream. Do not replace it with a reseeded source or a direct configured-feature call: modifiers on the nested placed feature are part of its behavior. When adding a new hanging-provider state family, add its explicit survival rule in `hanging_state_can_survive`; unknown families must not silently place. Root-system parsing requires every numeric codec field and its bounded range; malformed data degrades to `Unsupported` instead of defaulting a value that could alter placement or RNG consumption.

The support check uses the resolver's exact per-state solidity census when one is available. Small standalone grids may omit that census and use the documented base-id fallback; keep both paths aligned when the state model changes.
