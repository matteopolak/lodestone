...
The desktop WASM plugin host exposes copied local-player look, movement, block-breaking, and one-shot placement intents without giving a guest a world handle. A guest can install a yaw/pitch target, override one tick's input axes and buttons, start or abort a block break, request a block placement, swap a live menu slot with a hotbar position, throw one item or a complete stack from a live menu slot, or drop its carried cursor stack while native systems retain simulation and network ownership.
