...
With `observe:inventory`, the host also copies container content and slot changes, menu-local data, screen open/close, mount-screen, held-slot, and cursor-item events into value-only records. Each record carries the window or container identity and only the item key/count projection; menu state, mutable cursors, and packet construction stay in the native client. The `inventory-menu` sample guest and its composed-client tests cover the positive path and the default-denied control.
