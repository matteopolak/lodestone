...
| `status_json_model` | 1 | the raw status JSON captured from a real 26.2 server in `crates/versions/26.2/tests/fixtures/vanilla_status_response_26_2.json`; the target's expected fields are derived independently with `serde_json`, and the unit negative control proves a wrong player count is rejected |
...
fail, so an always-accepting oracle cannot pass silently.
