...
deserialization, chat-text JSON/NBT, and server-list status JSON. Most targets
find panics, hangs and decode crashes on malformed input; `status_json_model`
also checks parsed fields against an independent JSON model. **Track B**
...
Fifteen targets, each a `[[bin]]` in `fuzz/Cargo.toml` under
`fuzz/fuzz_targets/`:
...
| `status_json_model` | `lodestone_net::parse_status_json`, checked against an independent `serde_json::Value` model for player counts, samples, version fields and simple MOTDs | `lodestone-net` |
...
| `status_json_model` | 2 | one raw status JSON captured from a real 26.2 server plus the CI-minimized duplicate-key regression; the target's expected fields are derived independently with `serde_json`, and the unit negative control proves a wrong player count is rejected |
...
under an id that never carries it. The status target additionally runs the
captured response through an independent field model; its unit negative control
deliberately changes the external player count and requires the comparison to
fail, so an always-accepting oracle cannot pass silently. The duplicate-key
minimization found by CI is retained as `crates/lodestone-net/tests/fixtures/status_duplicate_sample.json`
and has a parser regression test: valid JSON objects with duplicate keys remain
documents, with the last occurrence providing the field value.
...
fifteen targets, then **at most 30 seconds or 10,000 executions per gated
...
Leak detection is off (`-detect_leaks=0`). The decoder targets' documented
property is "decoding attacker-controlled bytes must not panic"; a decoder that
allocates and then returns `Err` is a correct decoder, not a finding. The
`status_json_model` target also treats disagreement with its independent scalar
field model as a finding, while invalid JSON remains an expected error.
