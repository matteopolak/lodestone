...
attack/knockback/attack-strength-ticker/hurt-overlay all already landed),
`docs/view-bobbing.md`. Vanilla source is `.cache/mc/26.2/{src,client-src}`,
all citations below read directly from source, not from memory.
...
| vanilla mechanic | jar reference | our status | our reference |
|---|---|---|---|
| Attack-strength ticker + delay curve | `Player.tick`/`Player.getCurrentItemAttackStrengthDelay` (`attackStrengthTicker++`; delay `= 20/attack_speed`) | **correct** | `crates/lodestone-ecs/src/player.rs` (`AttackStrengthTicker`), `crates/lodestone-shell/src/sim/session.rs::attack_strength_delay`/`attack_strength_scale` |
| Crosshair cooldown indicator | `Hud.extractCrosshair` | **correct** (CROSSHAIR variant only; HOTBAR/ready-icon deliberately cut) | `crates/lodestone-shell/src/hud.rs` (`HudFrame::attack_cooldown`) |
| Damage computation, crit/sweep math, knockback amount, i-frames | `Player.attack`, `LivingEntity.hurtServer` | **N/A — correctly not built**: wire `Attack` packet carries only target id, damage is 100% server-authoritative | `docs/combat.md` confirms; verified the packet shape carries no damage field |
| Knockback applied to *local player* on being hit | `Entity.lerpMotion` (unconditional velocity replace) | **correct** | `apply_entity_velocity`, `crates/lodestone-ecs/src/ingest.rs` — writes into `PhysicsState.0.velocity` |
...
| Knockback-resistance attribute, sprint-knockback bonus (+0.5, `PLAYER_ATTACK_KNOCKBACK` sound) | `LivingEntity.knockback`, `Player.attack` | **N/A — server-side only**, nothing to build; server sends the final velocity and a broadcast sound either way | — |
| Invulnerability window / "only a stronger hit re-hurts" | `LivingEntity.hurtServer` | **N/A — server-side only**, only changes the damage amount sent, no client mechanic needed | — |
| `HurtTime`/`hurtDuration` countdown (drives red overlay + `bobHurt`) | `LivingEntity.animateHurt`, `LivingEntity.handleDamageEvent` | **correct**, both event sources wired | `HurtTime` component + `tick_hurt_time`, `crates/lodestone-ecs/src/ingest.rs`/`entity.rs` |
| Per-entity hurt/death red overlay (`hasRedOverlay`) | `LivingEntityRenderer.extractRenderState`, `OverlayTexture.java` | **correct, reaches pixels** — landed inside `812eb67`, documented at `ce6224d` after a staging mishap; **not stale**, verified live in tree | `crates/lodestone-render/src/entity_pipeline.rs` (`EntityInstanceRaw::with_hurt_overlay`), wired from `crates/lodestone-shell/src/entities.rs`/`gpu.rs` |
| — same overlay, `deathTime` half | `LivingEntityRenderer.extractRenderState`'s `\|\| entity.deathTime > 0` | **absent, small** — nothing decodes a death animation, so overlay ends ~10 ticks after the killing blow instead of persisting through the death flop. Documented, not a surprise. | — |
| `bobHurt` — camera roll toward `hurtDir` | `GameRenderer.bobHurt` | **island** — mechanism built and unit-tested (`ViewBob::hurt`, `BobFrame::hurt_roll_degrees`), **zero production callers**, and `Sim::render_camera` hardcodes `damage_tilt_strength = 0.0`. Blocked on `Camera` gaining a 4th (roll) DOF — a real architectural blocker, not a forgotten call. | `crates/lodestone-shell/src/camera_rig.rs` (`ViewBob::hurt`, called only from its own tests); `crates/lodestone-shell/src/sim/camera.rs::Sim::render_camera` (hardcoded `0.0`) |
| Crit condition (`canCriticalAttack`) + `1.5×` damage + `CRIT` particle | `Player.attack`/`Player.canCriticalAttack`; particle spawn is **client-only local prediction**, `LocalPlayer.crit` — never sent by the server | **absent** — no code anywhere computes the client-side crit condition. Genuinely needs building (not a wiring bug): damage math itself is still server-side, only the particle-trigger condition needs porting. | — |
| Sweep-attack condition + `SWEEP_ATTACK` particle + sound | `Player.attack`/`Player.isSweepAttack`/`Player.doSweepAttack` | **unverified, likely partially working already** — `minecraft:sweep_attack` IS a registered particle id (`crates/lodestone-data/src/generated/particle_types.rs`) and the generic server→`ClientEvent::Particles`→emitter pipeline is real and wired (fixed as an island in `77cb3a5`/`d26c4e6`). Unlike crit, vanilla's sweep particle is server-sent (`serverLevel.sendParticles`, `Player.doSweepAttack`), so it may already render with no new code. **Not confirmed** — the `count == 0` call encodes a direction vector rather than a spread, which our particle decode may or may not special-case correctly; needs a live-oracle check, not new code, as the first step. | `forward`, `crates/lodestone-shell/src/net.rs` |
| Shield raise / hold-to-block | `LivingEntity.hurtServer` (`applyItemBlocking`), item is `useOnRelease()`-gated | **absent in practice — see finding #1 below**, not because blocking logic needs porting (it's 100% server-side) but because the client cannot currently *hold* a use-item state at all in the situations combat actually happens in | `crates/lodestone-shell/src/sim/actions.rs::Sim::use_item_live` |
| Bow/crossbow draw-then-release fire | `LivingEntity.updateUsingItem`→`LivingEntity.completeUsingItem`→`LivingEntity.releaseUsingItem` (gated on `useOnRelease()`) | **absent in practice — see finding #2 below** | `crates/protocol/v770/src/adapter/serverbound.rs::V770Adapter::encode_client_action` (encodes `ReleaseUseItem`, zero callers) |
| Attack sounds (weak/strong/crit/sweep/no-damage/knockback) | `Player.attack`/`Player.doSweepAttack`, via `playServerSideSound` → `level().playSound(null,…)`, broadcast | **already correct, no client work needed** — generic server-sound pipeline (`docs/sound-playback.md`) plays any broadcast sound; combat sounds are just broadcast sounds like any other | `crates/lodestone-shell/src/audio.rs::ShellAudio::play_sound` |
| Camera shake on nearby explosions | claimed by the original scoping pass | **not a real vanilla mechanic** — grepped `client-src` clean for `[Ss]hake`; only hit is an unrelated item-wobble in `ItemInHandRenderer.java`. `ClientExplosionTracker.java` only spawns particles, holds no camera reference. | `docs/combat.md`'s own "What is deliberately not built here" already says this |
...
`ClientAction::ReleaseUseItem` is fully defined (`crates/lodestone-model/src/action.rs`)
and encoded by **four** protocol adapters — v47, v340, v735, and v770
(`crates/protocol/v770/src/adapter/serverbound.rs::V770Adapter::encode_client_action`, `PLAYER_ACTION` action id `5`,
...
auto-completes a use **only** when
...
(`LivingEntity.releaseUsingItem`/`LivingEntity.stopUsingItem`). Right now:
...
Independent of Finding 1, `use_item_live` (`crates/lodestone-shell/src/sim/actions.rs::Sim::use_item_live`)
...
Vanilla's `Minecraft.startUseItem` (`.cache/mc/26.2/client-src/…/Minecraft.java`)
...
`gameMode.interact` returns `PASS`) hits an explicit `break;` and falls
through to the unconditional generic-use call
...
`crates/lodestone-shell/src/sim.rs`), hostile or not — always short-circuits to `interact_entity` and returns,
...
2. **Crit particles.** Genuinely absent, needs building from scratch — but cheap: the
   condition (`Player.canCriticalAttack`) needs `fall_distance > 0` (already tracked,
   `PhysicsState.fall_distance`, `crates/lodestone-physics/src/player.rs`),
   ...
   (`crates/lodestone-shell/src/sim/actions.rs::Sim::attack_entity`), which already computes `attack_strength_scale` for the ticker
   ...
   own client-only spawn (`LocalPlayer.crit`), so it needs **no** new wire
   event.
...
3. **Verify sweep-attack particle before writing any code for it.** Given the
   ...
   (`Player.doSweepAttack`'s `sendParticles(SWEEP_ATTACK, x,y,z, 0, dx,0,dz,0)` — the
...
4. **Wire `bobHurt`'s production half**, once someone is willing to give `Camera` a
   ...
   `damage_tilt_strength` instead of the hardcoded `0.0` in `crates/lodestone-shell/src/sim/camera.rs::Sim::render_camera`). Lower
   ...
   - **Gate**: `docs/view-bobbing.md` already names
     ...
     computed from constants in `GameRenderer.bobHurt`, not against our own
     prior output.
...
6. **Correct `docs/backlog.md` item 7 and the Tier 1 issue tracker.** Strike
   "attack-strength cooldown bar" and "hurt tint" (both already shipped) and
...
- **Logic island, not a routing-switch island**: `ViewBob::hurt`/
  ...
  `camera_rig.rs`, called only by its own tests, never
  from production. Distinct second problem: even if wired, `Sim::render_camera`
  passes a hardcoded `0.0` for `damage_tilt_strength`
  (`crates/lodestone-shell/src/sim/camera.rs::Sim::render_camera`), so wiring the call alone would still show nothing — both
...
- Server-sent knockback applied to the local player's own `PhysicsState.velocity`
  ...
  unconditional-replace semantics.
...
- Knockback-resistance attribute, sprint-knockback bonus, invulnerability
  ...
  `LivingEntity.hurtServer`/`LivingEntity.knockback` and `Player.attack`; nothing to
...
- `HurtTime` countdown, both `EntityDamaged` and `EntityHurtAnimation` producers,
  ...
  (`crates/lodestone-ecs/src/ingest.rs`) — **not** stale, re-checked
...
- Attack-strength ticker, delay curve, crosshair cooldown indicator — correct,
  including the correct non-constant delay
...
- `git show` on `ce6224d` (the doc-pointer commit) to recover the hurt-overlay
  landing story that a plain `git log --grep` search by issue number alone
  would have missed (per the exact trap `CLAUDE.md` names for that commit).
