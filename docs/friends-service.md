...
`menu::friends` is the presentation consumer. The title and pause-menu Friends buttons open one three-tab surface: Friends lists established relationships, Pending combines incoming and outgoing requests, and Settings exposes the selected account's service-backed Friends availability and request permission. The Online settings page's `Friends List...` and `Allow Requests...` controls enter that same account-scoped Settings tab, while its `In-Game Notification` and `Visibility` rows are local, persisted controls. `PresenceSharing::All` publishes the current world type, `Limited` publishes only online, and `None` publishes offline. In-world relationship toasts are shown only when the notification option is enabled; title and menu toasts remain eligible. Established-Friend rows show the service's latest credential-free presence as trailing text when a row is available; request rows deliberately remain blank because relationship membership does not establish an activity state. The Pending tab carries a bounded incoming-request badge (`1` through `5+`) only after a usable snapshot is loaded, so unresolved, disabled, and never-loaded views do not present a fabricated zero. It renders loading, failure, empty, cached-list, disabled, and preference-saving states from `FriendsView`; outbound values are refresh, add-by-profile-name, `Accept`, `Decline`, `Cancel`, `Remove`, and a complete `FriendsPreferences` replacement. A successful relationship mutation is carried through the credential-free view and rendered as a specific inline confirmation; failures retain the confirmed relationship state and use their classified error copy, including separate unknown-profile, unavailable, rate-limit, privacy, rejected, invalid-response, expired-session, and signed-out notices. The Friends tab's Add Friend footer enters a bounded profile-name prompt and emits `FriendMutation::SendByName` through the existing worker boundary, so the request is both visible and connected to the service. The menu disables service settings until attributes arrive and while a save is in progress, so repeated clicks cannot replace an earlier preference request.

The rebindable `key.friends` action opens that same surface from active gameplay and defaults to `O`.
...
- Local Friends options: `in_game_notification` defaults to `false`; `share_presence` defaults to `all` and accepts `all`, `limited`, or `none` in `options.json`.
