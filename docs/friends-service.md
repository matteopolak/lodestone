...
`app::friends::FriendsApp` is the execution boundary around that coordinator. Native builds run it on a named worker with its own current-thread async runtime; browser builds use a local-task-compatible handle. Both receive only account metadata and activity intent, then return only `FriendsView` snapshots. Selected-account resolution, cache refresh, service calls, and the resolved session stay private to the executor. The frame calls `FriendsApp::sync` with the account switcher's selected profile, game activity, and local presentation choices; it sends only changes (re-sending the projected activity after an account switch), filters late views from a prior account, and gives menus a credential-free `view` accessor.

`menu::friends` is the presentation consumer. The title and pause-menu Friends buttons open one three-tab surface: Friends lists established relationships and includes a persistent profile-name field wired to `FriendMutation::SendByName`, Pending combines incoming and outgoing requests and shows their count in the tab label, and Settings exposes the selected account's service-backed Friends availability and request permission alongside the local in-world notification toggle and presence visibility cycle. Established-Friend rows show the service's latest credential-free presence as trailing text when a row is available; request rows deliberately remain blank because relationship membership does not establish an activity state. It renders loading, failure, empty, cached-list, disabled, preference-saving, typed mutation-success, and typed mutation-error states from `FriendsView`; outbound values are refresh, profile-name request, `Accept`, `Decline`, `Cancel`, `Remove`, and complete service or local preference replacements. The menu disables service settings until attributes arrive and while a save is in progress, while local choices persist eagerly for the selected profile. Presence visibility is `Full` by default, with `Limited` reducing activity to online/offline and `Hidden` publishing Offline. In-game notifications default off and suppress only HUD relationship toasts while a world is open; title-screen notifications remain available.

`FriendsNotificationFeed` consumes only successive credential-free `FriendsView` values. `FriendsApp` turns its emitted relationship changes into a short-lived HUD toast when the shared top-right toast slot is free, using the profile-keyed local toggle only at the presentation boundary. It has no token or service handle.
...
Keep relationship-delta interpretation in `FriendsNotificationFeed`, not in HTTP decoding or a menu callback. Seed a newly selected account before notifying, preserve the `outgoing`-to-`friends` requirement for acceptance, and keep the local in-game notification choice in the profile-keyed store/app boundary rather than in the service model or feed. The shared HUD toast slot has established higher-priority producers; leave a Friends toast queued until that slot is available instead of counting down while it is hidden.

The profile-name field is a real persistent `EditBox`, not a label that is rebuilt each frame. `MenuNav::key_friends` forwards character and editing events to it, while Enter or the Send Request footer queues a trimmed `SendByName` intent. An empty value is rejected locally as `InvalidInput`. A successful service mutation returns the authoritative snapshot plus a typed `FriendsMutationSuccess`; `menu::friends` maps `UnknownProfile`, `Unavailable`, `InvalidInput`, `MalformedResponse`, and `Rejected` to distinct notices so a failed request is not presented as a generic refresh failure.
...
When changing the menu, keep `MenuNav::refresh_friends_view` as the one app-to-menu copy and drain `MenuNav::take_friends_intents` at the app boundary. `FriendsIntent::SetPreferences` must route to `FriendsApp::set_preferences`, never directly to `FriendsService`; local `SetLocalPreferences` intents carry their profile UUID and route to the profile-keyed store. The worker and coordinator retain the request floor, backoff, and no-replay rules. Do not make a renderer resolve an account or call the service, and do not turn repeated refresh clicks into immediate network traffic. The pause route must be drawn and hit-tested as an overlay over the paused world; the title route is a full menu screen. Online Options enters the same Friends Settings tab, preserving the title/pause return stack. Both escape and Done must return to the route that opened Friends.

`FriendsPreferencesStore` persists the local choices as a versioned `friends.json` document next to `options.json`, keyed by profile UUID. It contains no session, token, friend row, or service response data. Missing, malformed, or newer documents use defaults; writes are eager and a failed write is surfaced on the Friends Settings tab. Keep the store behind `crate::platform::store` so browser builds use the same local-storage seam as the other small JSON documents.
...
- Local preferences path: `friends.json` next to `options.json`, keyed by profile UUID.
- Local defaults: in-game notifications off; presence visibility `Full`.

## Dependencies

The service module depends on `reqwest` for HTTPS and headers, `serde`/`serde_json` for private wire shapes, `uuid` for profile identifiers, and the existing `lodestone-auth::Session` resolver boundary. The runtime depends on those public Friends types, `Session`, `uuid`, and the shell's portable clock adapter. Local preferences depend on `serde_json`, `uuid`, and `crate::platform::store`. Neither layer depends on a protocol family, persistent service response state, or any multiplayer invitation transport.
