...
genuinely stranded, each re-verified this pass against current source rather
than assumed from an older table — see "Why … stay `Ignored`" below;
`PADDLE_BOAT` is now also connected, moved out of this list this pass — see
the interaction/combat remainder row below) and `#268`'s thirteen (`SET_COMMAND_BLOCK`, `CHANGE_DIFFICULTY`,
...
comment while auditing this issue: `canDropItems`, the tutorial hooks, and — found and left
as a follow-up rather than fixed in this pass — `Slot.mayPickup`.
(`tryItemClickBehaviourOverride`'s bundle case is modelled as of issue #692, but only for the
`PICKUP` click type — vanilla itself never calls it from `QUICK_MOVE`/`THROW`/drag either, so
that is not a gap.) Every take in `container_click` succeeds
