...
The first production chunk adapter is intentionally narrower than the schema's eventual scope. `WorldStorage::write_dirty_chunk` converts a server `ChunkColumn` into one independently replaceable `ChunkRecord`: it emits a per-section local palette of built-in numeric block-state IDs, a one-to-fifteen-bit little-endian packed local-index stream, no light bytes, the complete 4×4×4 built-in-biome grid for each section, the separate 4×4 surface-biome grid, a retained 16×16 motion-blocking map when the source has one, and one empty-root named-NBT byte string for each resident block entity. Before writing, it requires every entity's NBT position to match its `ChunkColumn` tuple, lie within this record's horizontal column and vertical extent, and be unique. `WorldStorage::load_chunk` opens the same `NativeStore` index, validates the requested key, current game-data version, block and biome section sequences, packed lengths, palette indices, enum values, grid extents, heightmap range, and every NBT root's name, trailing bytes, position, extent, and uniqueness before reconstructing a real `ChunkColumn`. Malformed or custom data that does not fit this lossless NBT boundary is an error, never a dropped entity; opaque NBT that does fit is retained verbatim. The integrated-server methods are thin consumers of those two operations. The reopen test constructs a second server and reads blocks, biome answers, a deliberately non-uniform heightmap, a simulated block entity, and an opaque block entity through `ChunkColumn`, so this is not merely a segment-level round trip.

This is **not** a complete native world format yet. The adapter preserves every built-in three-dimensional biome cell, surface-biome answer, retained motion-blocking heightmap, and resident block entity that can be represented as a complete NBT root, but before writing it still refuses any column with structures, shaped-only generation, or undrained generation-spawn candidates. It also rejects a biome outside the current built-in enum census rather than coercing an extension biome into a built-in value. The server `ChunkColumn` has no light representation, so its schema light-byte fields remain refused on read rather than decoded and discarded; no native save currently claims to preserve light. The caller supplies `min_y` and `height` when loading because version 1 stores section coordinates but no dimension definition; minimum Y must be aligned to 16. The existing Anvil `RegionChunkSource` remains the live terrain/entity/metadata loader and is deliberately not redirected through this partial adapter.
