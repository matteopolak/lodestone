...
The schema crate is intentionally not a server consumer. The engine decision, its atomic commit/recovery rules, extension-table lifecycle, schema negotiation, migration policy, and Anvil loss preflight remain integration work. In particular, it neither promises rollback nor infers an atomicity design from the benchmark segment's limited crash-tail control.
