...
The first production chunk adapter is intentionally narrower than the schema's eventual scope. `WorldStorage::write_dirty_chunk` converts a server `ChunkColumn` into one independently replaceable `ChunkRecord`: it emits a per-section local palette of built-in numeric block-state IDs, a one-to-fifteen-bit little-endian packed local-index stream, no light bytes, the complete 4×4×4 built-in-biome grid for each section, and the separate 4×4 surface-biome grid. `WorldStorage::load_chunk` opens the same `NativeStore` index, validates the requested key, current game-data version, block and biome section sequences, packed lengths, palette indices, enum values, and grid extents, then reconstructs a real `ChunkColumn`; the integrated-server methods are thin consumers of those two operations. The reopen test constructs a second server and reads both blocks and biome answers through `ChunkColumn`, so this is not merely a segment-level round trip.

This is **not** a complete native world format yet. The adapter preserves every built-in three-dimensional biome cell and surface-biome answer, but before writing it still refuses any column with block entities, structures, a retained motion-blocking heightmap, shaped-only generation, or undrained generation-spawn candidates. It also rejects a biome outside the current built-in enum census rather than coercing an extension biome into a built-in value. On read it refuses extension values and stored light bytes, rather than decoding a partial column and discarding information on its next save. The caller supplies `min_y` and `height` when loading because version 1 stores section coordinates but no dimension definition; minimum Y must be aligned to 16. The existing Anvil `RegionChunkSource` remains the live terrain/entity/metadata loader and is deliberately not redirected through this partial adapter.
