...
When an ore trace consumes one more exposure draw than the external server, first compare the pre-feature block at the exact candidate. The focused control in `crates/lodestone-worldgen/tests/overworld_ore_terrain_control.rs` records this distinction for seed `42`: the final coal blob candidate at `(-145,31,-142)` is `minecraft:andesite` externally, while a local `minecraft:stone` value makes it look like a target and consumes the twentieth draw. A failing control is therefore evidence of an upstream terrain mismatch; changing the ore matcher or RNG to hide the extra draw is the wrong repair. Keep the external block value independent of the generator under test and retain the coordinate in any follow-up terrain comparison.
