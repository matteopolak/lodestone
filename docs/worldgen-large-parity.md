...
Set `LODESTONE_LARGE_PARITY_STREAM_SCAN_ALL=1` to consume the complete selected
stream and retain an exact mismatch inventory instead of stopping at the first
record. `LODESTONE_LARGE_PARITY_STREAM_ARTIFACT_DIR` selects the output
directory; if omitted, the comparator uses
...
heightmap, and block-entity differences. End packet records additionally
classify packet, sidecar, and decoded content differences. Use a fresh artifact
directory for each run when comparing results across code revisions.
An explicit artifact directory must be new or empty; existing evidence is
never overwritten.
...
The normal stream is intentionally ephemeral: there is no resume prefix or
packet dump, and the container bridge is removed on exit. Scan-all is the
explicit diagnostic exception and writes only the bounded artifact set above;
the bridge remains disposable in either mode. Defaults select `(0,0)`; pass
`--cx` and `--cz` for a bounded batch. A mismatch fails with coordinate and
digest information. Set `LODESTONE_LARGE_PARITY_STREAM_DIAGNOSTICS=1` only when
a first differing record offset is needed; normal mismatches do not rerun
lifecycle materialization or write temporary packets.
