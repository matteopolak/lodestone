...
The dedicated host routes scalar and batch Java replacements through the running server's proposal bus, so native-plugin adjudication can deny or replace a request before the adapter reports success. The batch action carries ordered replacements and the listener-notification policy as one unit; the server preflights every target before mutating the first one and returns the adjudicator's resolved batch to the adapter. The proposal path also returns finite `ColumnNotResident` and `OutOfBounds` refusals. A batch is never split into scalar proposals, which would allow partial application after a later denial.

The ignored `paper_lifecycle::plugin_child_reads_and_writes_resident_block_state_through_worker_ports` fixture is deliberately non-Paper: it compiles repository-owned stand-in archives and drives a bounded in-memory state map. It exercises scalar and ordered batch writes, both notification policies, and scalar/batch read-after-write through a real JDK and JNI. The fixture also sends a malformed block handle and requires the named lifetime failure; the separate adapter-host JDK fixture covers a genuinely stale player handle after disconnect and slot reuse. These tests validate JNI and worker ownership, not Paper compatibility.
