...
The dedicated host routes scalar Java replacements through `IntegratedServer::set_resident_block_state_proposed`, so the running server's native-plugin adjudication can deny or replace a request before the adapter reports success. The proposal path also returns finite `ColumnNotResident` and `OutOfBounds` refusals. Batch replacements retain their all-or-nothing resident preflight and direct application until the proposal vocabulary has a batch action; splitting a batch into scalar proposals would allow partial application after a later denial.
