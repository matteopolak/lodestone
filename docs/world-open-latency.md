...
| `serve_play.rs`: `a_play_packet_is_serviced_before_the_last_join_chunk` | a difficulty change sent immediately after configuration is answered after **< 40** of 361 join chunks — measured **21 / 12 / 24** over three runs — and the view still arrives whole and in order | **361** — the reply cannot precede the last chunk, because the loop that produces it has not started |
