...
impl DefaultTints {
    ...
    /// `minecraft:grass` tint names its own `temperature`/`downfall` in its own
    /// item definition — vanilla's six files all say plains, so the two agree
    /// today, but the item path must sample the climate the JSON asked for
    /// rather than assume it. Hence the raw map, not a pre-sampled colour.
