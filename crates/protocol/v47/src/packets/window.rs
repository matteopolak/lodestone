...
//!
//! Byte-identical to v340's own window packets (measured: only doc comments
//! differed). `CloseWindow`, `EnchantItem`, `HeldItemSlot`,
//! `ServerboundCloseWindow` and `ServerboundHeldItemSlot` carry no `Slot`
//! field and are shared with v735 too. `OpenWindow`, `SetCreativeSlot`,
...
//! are shared with v340 only -- see `lodestone-protocol-common`'s
