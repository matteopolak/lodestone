...
//! action-*bitmask* per entry), 1.8 has a single `player_info` packet whose
...
//! `pc/1.8/protocol.json` `packet_player_info` type — a `mapper`-tagged
...
//! 1.8 chat field (`ClientboundChat::message`) — 1.8 predates the network-NBT
//! chat-component wire format modern versions use. Decoded to [`Text`] at
//! the adapter layer via `Text::from_json`, matching that packet's
//! convention, not here.
...
/// Maximum property name/value/signature lengths — 1.8 has no dedicated
...
/// Clientbound `player_info` (id 56 in this family's table).
