//! Hand-written codec for the 1.8 packed block `position` type.
//!
//! # Architectural finding (macro gap)
//!
//! Every other field in this crate is expressed with ordinary struct fields and
//! the derive macro. The 1.8 `position` type cannot be, because it is a
//! *bit-packed* value: three signed integers (x: 26 bits, y: 12 bits, z: 26
//! bits) packed into a single big-endian `i64`. The `#[mc(...)]` attribute set
//! (`varint`, `varlong`, `len`, `max`, `fixed`, `remaining`, ...) has no way to
//! describe a sub-byte bitfield, so this is the one place a hand-written
...
//! general `#[mc(bits(x = 26, y = 12, z = 26))]`), because the *only* thing
//! that differs between 1.8 and modern positions is the field order of the
//! packing: 1.8 packs `x, y, z` (y in the middle) whereas 1.14+ packs
//! `x, z, y` (y in the low bits). A subtly wrong bit layout is invisible to a
//! round-trip test, which is why this module is covered by byte-level golden
//! tests.
...
/// Packs a [`BlockPos`] into the 1.8 `position` `i64` representation.
///
/// The 1.8 layout is `x(26) | y(12) | z(26)` from the most significant bit
/// down, i.e. y occupies the middle bits. This differs from the 1.14+ layout,
/// which packs `x(26) | z(26) | y(12)`.
#[must_use]
...
/// Unpacks a 1.8 `position` `i64` into a [`BlockPos`], sign-extending each of
...
/// A block position encoded using the 1.8 packed `position` wire type.
///
