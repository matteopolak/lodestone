...
//! `lodestone-v340`'s own `tests/block_updates.rs` uses for the identically-
//! shaped 1.12.2 packets, and `lodestone-v770`'s `tests/block_updates.rs` uses
...
use lodestone_v47::packets::position::pack_position;
use lodestone_world::{
...
fn multi_block_change_rejects_chunk_coordinate_outside_the_world_border() {
    // Regression fixture for the exact hazard `lodestone-v340`'s
