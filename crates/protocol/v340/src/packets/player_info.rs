...
//!
//! Unlike 26.2's split `player_info_update`/`player_info_remove` pair (an
//! action-*bitmask* per entry), 1.12.2 has a single `player_info` packet whose
//! **one leading `action` varint applies to every entry in the packet**:
//! `0` = add (name, properties, game mode, ping, display name all at once),
//! `1` = update game mode, `2` = update latency, `3` = update display name,
//! `4` = remove. Confirmed against `vendor/minecraft-data`'s
//! `pc/1.12.2/protocol.json` `packet_player_info` type — a `mapper`-tagged
//! `action` varint followed by a `switch` on that same field, one schema
//! per action. That whole-packet-shared discriminant (rather than a
//! per-entry bitmask) is why this needs a hand-written decoder: the derive
//! macros' `#[mc(when = ...)]` expresses a field gated on a sibling field
//! within the *same* struct, not a shape shared by every element of an
//! array.
//!
//! **This is byte-identical to 1.8's `player_info` shape** — diffed both
//! versions' `packet_player_info` type in `minecraft-data`'s `protocol.json`
//! field by field rather than assuming 1.12.2's tab list carries the modern
//! per-entry action bitmask. It does not; that split arrives later. So this
//! module is a near-verbatim port of `lodestone-v47::packets::player_info`,
//! not an independent design.
//!
//! Display names travel as a bare JSON string here, the same as every other
//! 1.12.2 chat field (`ClientboundChat::message`) — 1.12.2 predates the
//! network-NBT chat-component wire format modern versions use. Decoded to
//! [`Text`] at the adapter layer via `Text::from_json`, matching that
//! packet's convention, not here.
...
/// Maximum property name/value/signature lengths — 1.12.2 has no dedicated
/// property codec limit, so this uses the wire's own general string cap.
...
/// Clientbound `player_info` (its id in this family's table).
