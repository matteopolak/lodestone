//! Hand-written codec for the pre-1.14 packed block `position` type.
//!
//! # Architectural finding (macro gap)
//!
//! Every other field in this crate is expressed with ordinary struct fields and
//! the derive macro. The pre-1.14 `position` type cannot be, because it is a
...
//! packing: 1.8 through 1.13 (protocol 340 is 1.12.2) pack `x, y, z` (y in the
//! middle) whereas 1.14+ packs `x, z, y` (y in the low bits). A subtly wrong
