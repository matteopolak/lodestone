//! Selection tests for 1.12.2's `EntityPlayerSP.onUpdateWalkingPlayer`.
...
fn encode(adapter: &V340Adapter, action: ClientAction) -> Option<(i32, Vec<u8>)> {
    adapter
...
#[test]
fn vanilla_1_12_selects_position_look_position_and_look_by_dirty_axes() {
...
#[test]
fn vanilla_1_12_is_quiet_when_idle_but_reports_an_on_ground_transition() {
    let adapter = V340Adapter::new();
...
#[test]
fn vanilla_1_12_forces_position_on_the_20th_idle_tick() {
    let adapter = V340Adapter::new();
    encode(&adapter, action(BASE_POS, BASE_ROT, true));
    for _ in 0..19 {
        assert_eq!(encode(&adapter, action(BASE_POS, BASE_ROT, true)), None);
...
#[test]
fn vanilla_1_12_ignores_sub_threshold_position_jitter() {
    let adapter = V340Adapter::new();
    encode(&adapter, action(BASE_POS, BASE_ROT, true));
...
#[test]
fn vanilla_1_12_uses_a_strict_nine_e_minus_four_position_boundary() {
    let adapter = V340Adapter::new();
...
#[test]
fn fresh_1_12_adapter_starts_from_a_zero_baseline_and_clones_share_it() {
    let adapter = V340Adapter::new();
    let zero = Vec3::default();
    let rotation = Rotation::default();
    assert_eq!(encode(&adapter, action(zero, rotation, false)), None);
