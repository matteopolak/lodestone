//! Hermetic tests for the protocol 340 (Minecraft 1.12.2) `block_change` and
...
//! `lodestone-v770`'s own `tests/block_updates.rs` uses for the modern
//! `block_update`/`section_blocks_update` packets. Golden byte vectors are
//! hand-assembled from minecraft-data's 1.12.2 `protocol.json` (identical to
//! 1.8's own shape for both packets), never from this crate's own `Encode`
//! derive, so a symmetric encode/decode bug cannot pass silently.
//!
//! The expected canonical block-state ids come from `lodestone_v340::canonical
//! ::resolve_or_air` directly — that bridge is independently verified
//! elsewhere (against the real 1.13.2 server jar's own flattening fix; see
//! `src/canonical.rs`'s module docs) and is used here purely as the
//! already-established oracle for "what should this `(id, meta)` pair
//! resolve to", not as part of the code path under test (single/multi block
//! writes, not chunk-column decode).
//!
...
use lodestone_v340::canonical::{self, FallbackTally};
use lodestone_v340::packet_ids::play;
use lodestone_v340::packets::position::pack_position;
use lodestone_world::{
...
fn block_change_routes_stone_to_canonical_state() {
    ...
    // (old_block_id=1 "stone", meta=0) -> composite id 16.
    let mut payload = pack_position(lodestone_model::BlockPos::new(10, 64, -5))
        .to_be_bytes()
        .to_vec();
    ...
    assert_eq!(sink.set_block, vec![(10, 64, -5, expected_state)]);
    assert_eq!(sink.sync_block_entity, vec![(10, 64, -5, None)]);
    // (10, 64, -5) is section-relative (10, 0, 11) in section (0, 4, -1).
    assert_remesh(
        &directives,
        &[(SectionPos::new(0, 4, -1), &[[10, 0, 11]])],
    );
}
...
fn multi_block_change_routes_records_within_one_section() {
    ...
    // rel (5,64,9), both "stone" (composite id 16).
    ...
    assert_remesh(
        &directives,
        &[(SectionPos::new(2, 4, -3), &[[1, 0, 2], [5, 0, 9]])],
    );
}
