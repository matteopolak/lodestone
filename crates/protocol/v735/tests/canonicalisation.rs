...
//!
//! ```text
//! LODESTONE_REGEN=1 cargo test -p lodestone-v735 --test canonicalisation \
...
fn generate(states: &[SourceState]) -> String {
    ...
    out.push_str(
        "// @generated by `cargo test -p lodestone-v735 --test canonicalisation -- --ignored`\n\
...
fn committed_table_is_internally_consistent() {
    assert_eq!(
        lodestone_v735::generated_canonical::STATE_TO_CANONICAL.len(),
        lodestone_v735::generated_canonical::SOURCE_STATE_COUNT as usize
    );
    for &id in lodestone_v735::generated_canonical::STATE_TO_CANONICAL.iter() {
        assert!(
    ...
    let air = lodestone_v735::generated_canonical::AIR_STATE_ID;
...
fn discriminating_states_resolve_to_their_26_2_ids_not_their_wire_ids() {
    ...
    let mut tally = lodestone_v735::canonical::FallbackTally::default();
    for &(label, wire_id, expected_26_2_id, wrong_name) in cases {
        let resolved = lodestone_v735::canonical::resolve_or_air(wire_id, &mut tally);
        assert_eq!(
