...
//! action-*bitmask* per entry), 1.16.5 has a single `player_info` packet whose
...
//! `pc/1.16.2/protocol.json` `packet_player_info` type — a `mapper`-tagged
//! `action` varint followed by a `switch` on that same field, one schema per
//! action, byte-identical to `lodestone-v340`'s (1.12.2) own shape. That
//! whole-packet-shared discriminant (rather than a per-entry bitmask) is why
//! this needs a hand-written decoder: the derive macros' `#[mc(when = ...)]`
//! expresses a field gated on a sibling field within the *same* struct, not a
//! shape shared by every element of an array.
//!
//! Display names travel as a bare JSON string here — decoded to [`Text`
//! ](lodestone_model::Text) at the adapter layer via `Text::from_json`,
//! matching every other 1.16.5 chat-shaped field, not here.
...
/// Maximum property name/value/signature lengths — this protocol has no
/// dedicated property codec limit, so this uses the wire's own general string
/// cap.
