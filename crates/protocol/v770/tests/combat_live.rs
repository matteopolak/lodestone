...
///   hand-derivation `lodestone-server/tests/mob_attack.rs`'s
///   `positive_knockback_power_produces_the_exact_predicted_velocity` pins),
///   applied as a one-tick position displacement
///   (`NavigatingMob::apply_knockback`), so the target's expected post-hit
///   position is `(1 - 0.5, 64 + 0.4, 0 + 0.0) = (0.5, 64.4, 0.0)`.
#[tokio::test]
async fn real_client_attacks_a_live_mob_and_the_server_applies_damage_and_knockback() {
    ...
    // Sprinting, so this attack's knockback power is the real jar constant
    // `0.5`, not `0.0` — see `SPRINT_ATTACK_KNOCKBACK_POWER`'s own doc
    // comment for why a *non*-sprinting bare-handed attack's power is
    // correctly zero and would make this assertion vacuous.
    handle
    ...
    let expected_pos = Vec3::new(0.5, 64.4, 0.0);
