//! `minecraft:resource_location` — vanilla's own identifier argument, `/stopwatch`'s
//! `<id>` and `/execute … stopwatch <id>`'s own argument.
//!
//! # No registry to validate against, unlike [`crate::BiomeArg`]/[`crate::BlockArg`]
//!
//! A stopwatch id is a user-chosen name (vanilla's own stopwatch-creation
//! routine stores whatever the caller typed), not a lookup into a fixed census — the
...
/// Vanilla's own identifier argument — `minecraft:resource_location`.
