...
It emits one Rust shard per scenario under ``tests/support/golden_traces/``.
Each shard contains that scenario's per-tick position and velocity as raw
``f64`` bit patterns (so no decimal rounding can creep in). The Rust integration
test replays the same scenarios through the crate and asserts bit-for-bit
equality. ``--check`` verifies that every expected shard exists, no stale Rust
shard remains, and every generated byte matches this oracle.
...
OUTPUT_DIR = Path(__file__).resolve().parent / "support" / "golden_traces"
...
def render_trace(name, trace):
    const = "GOLDEN_" + name.upper()
    out = [f"/// Golden trace for the `{name}` scenario ({len(trace)} ticks)."]
...
def generated_outputs():
    """Return every generated relative path and its exact UTF-8 contents."""
    traces = []
    ...
    for name, trace in traces:
        outputs[f"{name}.rs"] = "\n".join(
            render_preamble() + ["", "use super::GoldenTick;", ""] + render_trace(name, trace)
        ) + "\n"

    module = render_preamble() + ["", *render_tick_type(), ""]
    for name, _ in traces:
        module.append(f"mod {name};")
    module.append("")
    for name, _ in traces:
        const = "GOLDEN_" + name.upper()
        module.append(f"pub use {name}::{const};")
    outputs["mod.rs"] = "\n".join(module) + "\n"
    return outputs


def check_outputs(outputs):
    """Check missing, stale, and byte-different generated shards."""
    expected = set(outputs)
    actual = {path.name for path in OUTPUT_DIR.glob("*.rs")} if OUTPUT_DIR.is_dir() else set()
    missing = sorted(expected - actual)
    stale = sorted(actual - expected)
    mismatched = sorted(
        path for path in expected & actual if (OUTPUT_DIR / path).read_text() != outputs[path]
    )
    if missing:
        print("missing golden trace shards:", ", ".join(missing), file=sys.stderr)
    if stale:
        print("stale golden trace shards:", ", ".join(stale), file=sys.stderr)
    if mismatched:
        print("drifted golden trace shards:", ", ".join(mismatched), file=sys.stderr)
    return not (missing or stale or mismatched)


def write_outputs(outputs):
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    for relative, contents in outputs.items():
        (OUTPUT_DIR / relative).write_text(contents)
...
def main():
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="fail if generated shards are missing, stale, or drift from this oracle",
    )
    ...
    if args.check:
        if not check_outputs(outputs):
            return 1
        print(f"golden trace shards are up to date ({len(outputs) - 1} scenarios)")
        return 0
    ...
    print(f"wrote {len(outputs) - 1} golden trace shards to {OUTPUT_DIR}")
    return 0
