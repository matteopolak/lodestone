//! Focused production coverage for the Nether's soul-fire decoration.
//!
//! The expected cells are copied from an authenticated region capture. The
//! control withholds only the first `patch_soul_fire` placement entry, leaving
//! every other generation input unchanged.
...
const EXTERNAL: &str = include_str!("support/nether_soul_fire_external.txt");
const SEED: i64 = 42;
const CHUNK_X: i32 = 20;
const CHUNK_Z: i32 = 25;
...
struct WithoutSoulFire(ExternalAssets);

impl Resolver for WithoutSoulFire {
    fn density_function(&self, id: &str) -> Value {
    ...
    fn biome_document(&self, id: &str) -> Value {
        ...
        for entry in entries {
            if !withheld && entry.as_str() == Some("minecraft:patch_soul_fire") {
                *entry = Value::String("lodestone:withheld_patch_soul_fire".to_owned());
                withheld = true;
            }
        }
        document
...
fn external_fixture() -> ExternalFixture {
    ...
    assert_eq!(fixture.feature, "minecraft:patch_soul_fire");
    assert_eq!(fixture.block, "minecraft:soul_fire");
...
#[test]
fn soul_fire_config_is_present_in_the_bundled_nether_data() {
    ...
    assert_eq!(
        entries
            .iter()
            .position(|entry| entry.as_str() == Some("minecraft:patch_soul_fire")),
        Some(2)
    );
    let placed = assets.read("placed_feature", "minecraft:patch_soul_fire");
    assert_eq!(placed["feature"], "minecraft:patch_soul_fire");
    assert_eq!(placed["placement"][0]["count"]["max_inclusive"], 5);
    let configured = assets.read("configured_feature", "minecraft:patch_soul_fire");
    assert_eq!(configured["type"], "minecraft:simple_block");
    assert_eq!(
        configured["config"]["to_place"]["state"]["Name"],
        "minecraft:soul_fire"
    );
}

#[test]
fn external_soul_fire_cells_reach_the_production_nether_column() {
    let fixture = external_fixture();
    let assets = ExternalAssets::new();
    let settings = settings(&assets);
    let with_soul_fire = NetherGenerator::new(SEED, &settings, &assets);
    ...
    assert_eq!(
        ...
        "the production column's feature-only soul-fire cells differ from the authenticated external capture"
    );
    assert!(
        !produced.is_empty(),
        "the selected production column must exercise the soul-fire consumer"
    );
    assert!(
        fixture
            .cells
