...
pub struct EndGenerator {
    ...
    starts: Mutex<HashMap<(i32, i32), Arc<Vec<Arc<crate::structure::StructureStart>>>>>,
}
...
impl EndGenerator {
    ...
    fn structure_starts_stage(
        ...
        if let Some(existing) = self
            .starts
            .lock()
            .expect("end starts memo poisoned")
            .get(&(cx, cz))
        {
            return Arc::clone(existing);
        }

        // Compute outside the lock: start generation samples density columns
        // and assembles templates. A race computes the same pure value twice,
        // but never serialises unrelated chunk generation.
        let computed = Arc::new(match &self.structures {
            None => Vec::new(),
            Some(registry) => {
        ...
        let mut memo = self.starts.lock().expect("end starts memo poisoned");
