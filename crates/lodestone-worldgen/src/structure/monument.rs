...
//!   `Vec<RoomDef>` built by [`generate_room_graph`]; the graph edge writer
//!   writes both directions in one call, matching a faithful two-sided
//!   connection write.
...
//!   bump) entirely when the first call returns `false`. The graph closing pass
...
mod tests {
    ...
    /// The graph edge writer writes both sides of an edge — a
