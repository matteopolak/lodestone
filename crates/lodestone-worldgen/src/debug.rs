...
impl DebugColumn {
    ...
    pub fn top_non_air_y(&self) -> i32 {
        if self
            .grid_row
            .iter()
            .any(|&state| state != Block::Air.default_state())
        {
            GRID_Y
        } else {
            BARRIER_Y
...
impl DebugLevelSource {
    ...
    #[must_use]
    pub fn new(states: Vec<StateId>, min_y: i32, height: i32) -> Self {
    ...
    #[must_use]
    pub fn state_for(&self, world_x: i32, world_z: i32) -> StateId {
    ...
    pub fn column(&self, cx: i32, cz: i32) -> DebugColumn {
        ...
        let grid_row = std::array::from_fn(|i| {
            ...
            self.state_for(base_x + local_x, base_z + local_z)
        });
        DebugColumn {
            min_y: self.min_y,
            height: self.height,
...
mod tests {
    ...
    fn five_states() -> Vec<StateId> {
        (100..105)
            .map(|raw| StateId::new(raw).expect("fixture state id is valid"))
            .collect()
    }
    ...
    fn grid_dimensions_at_real_state_count() {
        let states: Vec<StateId> = (0..32366)
            .map(|raw| StateId::new(raw).expect("fixture state id is valid"))
            .collect();
    ...
    fn state_for_matches_hand_computed_indices() {
        ...
        let cases: Vec<((i32, i32), StateId)> = vec![
            ((1, 1), five_states()[0]),
            ((3, 1), five_states()[3]),
            ((1, 3), five_states()[1]),
            ((0, 1), air),
    ...
    fn column_places_barrier_and_grid_row_and_nothing_else() {
        ...
        for lx in 0..16 {
            for lz in 0..16 {
                ...
                if got != Block::Barrier.default_state() {
        ...
        if column.block_state(1, GRID_Y, 1) != five_states()[0] {
            mismatches.push(format!(
                "grid (1,1): expected state_0, got {:?}",
                column.block_state(1, GRID_Y, 1)
            ));
        }
        if column.block_state(0, GRID_Y, 0) != Block::Air.default_state() {
            mismatches.push(format!(
        ...
        for &y in &[-64, 0, 59, 61, 69, 71, 100, 319] {
            ...
            if got != Block::Air.default_state() {
    ...
    fn top_non_air_y_falls_back_to_barrier_when_grid_row_is_all_air() {
        ...
        let generator = DebugLevelSource::new(
            vec![StateId::new(100).expect("fixture state id is valid")],
            -64,
            384,
        );
