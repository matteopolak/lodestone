...
| `lodestone-plugin-support` | shared, non-engine conveniences: a per-plugin data directory and typed config helper (`paths`/`config`), live namespaced entity/chunk metadata (`persistent_data`), and bounded durable plugin/world/player/entity records (`durable_data`). |
