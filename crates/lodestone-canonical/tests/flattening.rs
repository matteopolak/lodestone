...
//! produced by reflectively reading the private static lookup array inside
//! class `yp` of the real 1.13.2 server jar's own `DataFixerUpper`
//! flattening fix — see `oracle-java/FlatteningOracle.java`'s module docs for
//! how that class was found, what it is and is not authoritative for, and
...
//!
//! 1. Re-dump (pure JDK, no Docker/live server needed — `yp`'s static
//!    initializer runs on class-load, no `Bootstrap`/`SharedConstants` call
//!    required):
//!
//! ```text
...
//!    jar). Note: if the jar changes, class `yp` almost certainly will not be
//!    named `yp` any more — obfuscated names are jar-build-specific, not
//!    stable across builds. Rediscover it with the grep-then-decompile method
//!    documented in `FlatteningOracle.java`'s class doc before touching
//!    anything else.
//!
...
/// is a TileEntity field, not derivable from meta at all. Verified against
...
/// and rotation are TileEntity fields). Detected mechanically wherever it
...
fn generate(rows: &[DumpSlot]) -> String {
    ...
    out.push_str(
        "// @generated by `cargo test -p lodestone-canonical --test flattening -- --ignored`\n\
         ...
         // 1.13.2 server jar's own DataFixerUpper flattening fix, protocol 340 /\n\
    ...
    let _ = writeln!(
        ...
        "//! Generated pre-Flattening `id:meta` -> block-state table for protocol 340\n\
         ...
         //! does not); {} require TileEntity/neighbor data this table cannot supply.",
        resolved_slot_count,
        classified.len(),
        distinct.len(),
