...
mod tests {
    ...
    fn vegetal_features_for(
        biome: &str,
    ) -> Vec<(usize, lodestone_worldgen::feature::vegetation::PlacedRef)> {
        let resolver = embedded_resolver();
        let catalog = lodestone_worldgen::compose::build_decoration_catalog(
            &resolver,
            &[biome.to_owned()],
        );
        catalog
            ...
            .filter_map(|(step, index, placed)| {
                (step == lodestone_worldgen::feature::STEP_VEGETAL_DECORATION)
    ...
    /// Remaining possible reasons cover unsupported tree configuration or
    /// feature forms such as coral, bamboo, root systems, cave vegetation,
    /// and simple
    /// block placement. The table is generated by running every reachable biome
    /// through the unified `lodestone_worldgen::compose::DecorationCatalog` and
    ...
    /// `kelp`, `seagrass`, `sea_pickle`, `vines`, `vegetation_patch`,
    /// `random_boolean_selector`, and both huge-mushroom variants are all
    /// implemented now and dropped off. The mushroom-fields selector is still
    /// resolved as a normal nested feature, so a regression in either branch
    /// returns its named reason here.
    ///
    ...
    fn vegetation_placer_gaps_are_named_not_silent() {
        ...
        for biome in names {
            let list = vegetal_features_for(&biome);
    ...
    fn vegetation_gap_mismatches_fires_on_an_undeclared_gap() {
        ...
        actual.insert(
            ...
            vec!["brand_new_unimplemented_placer".to_string()],
        );
        let mismatches = vegetation_gap_mismatches(&actual);
        assert!(
    ...
    fn vegetation_gap_mismatches_fires_on_a_gap_that_was_fixed() {
        let mut actual = KNOWN_VEGETATION_GAPS
            .iter()
            .map(|(biome, reasons)| {
                (
                    (*biome).to_string(),
        ...
        // Preserve every other known row, but report warm-ocean coral as fully
        // clean to simulate a fixed gap that was not pruned from the ledger.
        actual.insert("minecraft:warm_ocean".to_string(), Vec::new());
        let mismatches = vegetation_gap_mismatches(&actual);
        assert!(
            mismatches.iter().any(|m| m.contains("minecraft:warm_ocean")),
    ...
    #[test]
    fn decoration_catalog_resolves_plains_grass_flower_and_tree() {
        use lodestone_worldgen::feature::vegetation::{BlockStateProvider, ConfiguredFeature};

        let list = vegetal_features_for("minecraft:plains");
