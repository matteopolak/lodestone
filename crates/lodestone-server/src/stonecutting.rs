//! The stonecutter (issue #150, station half): a real, server-computed
//! recipe list from the stonecutting corpus [`crate::crafting::recipe_book`]
//! already loads — `StonecutterMenu`'s own `recipesForInput` /
//! `setupResultSlot`.
//!
//! # What it is
//!
//! Vanilla's `StonecutterMenu` keeps two pieces of state beyond the plain
//! grid: `recipesForInput` (every `StonecutterRecipe` whose ingredient the
//! current input item satisfies, recomputed on `slotsChanged` whenever the
//! input's *item* changes) and `selectedRecipeIndex` (a `DataSlot`, set by
//! `clickMenuButton` — [`crate::container_click`]'s `ContainerButtonClick`
//! consumer in `crate::server`). [`matches`] is the first; [`result`] folds
//! both into the one result vanilla's `resultSlot` shows.
//!
//! # How it works
//!
...
//! source.
//!
//! **Ordering is a disclosed approximation.** Vanilla's real order comes from
//! `RecipeManager`'s registration order, which this crate does not preserve
//! per-recipe (`recipe_book()` is a `HashMap`-shaped corpus, per its own
//! doc). [`matches`] sorts by recipe id instead — stable across calls (so a
//! `ContainerButtonClick`'s `button_id` keeps meaning the same recipe between
//! one `slotsChanged`-equivalent and the next) but not guaranteed to be
//! vanilla's exact button order. Getting the *exact* order would need a JVM
//! oracle dump the same way `EntityDataIndexOracle` exists for metadata
//! indices — nobody has built one for `RecipeManager`'s stonecutting order
//! yet.
//!
//! # How to change it
//!
...
/// necessarily vanilla's exact order) sequence — `StonecutterMenu`'s own
/// `recipesForInput`.
#[must_use]
...
/// whole list. `0` for no input, matching `StonecutterMenu.getNumberOfVisibleRecipes`
...
/// — `StonecutterMenu::setupResultSlot`'s `isValidRecipeIndex` guard, folded
