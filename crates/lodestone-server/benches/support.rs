//! Shared bench-recording helper for `lodestone-server`'s criterion benches.
//! A byte-for-byte sibling of the same file under `lodestone-worldgen`,
//! `lodestone-world`, `lodestone-render`, `lodestone-shell`,
//! `lodestone-entity`, `lodestone-physics` and the 26.2 protocol family,
//! minus each of their crate-specific guards.
//!
//! It is duplicated rather than shared because a `benches/` module cannot be
//! imported across crates without promoting it to a real workspace crate, and
//! nothing here depends on the crate it sits in. Eight copies is past the
//! point where that trade still favours duplication -- `lodestone-testsupport`
//! is already a dev-dependency of most of these crates and is the natural
//! home -- but moving it is a cross-crate change of its own, not a side effect
//! of adding a bench. See `docs/benchmark-regression-gate.md`.
//!
//! # What this does
//!
