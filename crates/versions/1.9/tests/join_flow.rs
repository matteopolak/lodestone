...
use lodestone_v1_9::packet_ids::{handshaking, login, play};
use lodestone_v1_9::packets::common::{KeepAliveRequest, KeepAliveResponse};
...
use lodestone_v1_9::packets::login::{
    EncryptionRequest, EncryptionResponse, LoginDisconnect, LoginStart, LoginSuccess,
    SetCompression,
};
use lodestone_v1_9::packets::position::{Position, pack_position, unpack_position};
use lodestone_v1_9::packets::status::{StatusPing, StatusPong, StatusRequest, StatusResponse};
...
fn encode_respawn_is_client_command_action_zero() {
    ...
    // minecraft-data's 1.12.2 protocol.json `packet_client_command`.
...
//
// Every payload below is hand-assembled straight from minecraft-data's
// 1.12.2 `protocol.json` field list (verified identical to 1.8's own shape
// for each of these four packets) rather than built with this crate's own
// `Encode` derive — the same "independent byte vector" standard
// `tests/join_flow.rs`'s module doc and `lodestone-v26-2`'s
// `tests/block_updates.rs` both already use, so a symmetric encode/decode
// bug in the derive macro cannot pass silently.
...
fn play_respawn_emits_respawned_event() {
    let adapter = V340Adapter::new();
    let mut payload = (-1i32).to_be_bytes().to_vec(); // dimension: nether
