...
async fn capture() -> String {
    let conn = Connection::connect(("127.0.0.1", 25604)).await.expect("connect oracle");
    capture_connection(conn).await
}

async fn capture_connection<T: lodestone_net::Transport>(mut conn: Connection<T>) -> String {
    let adapter = lodestone_v1_21_11::adapter_for(774);
    let mut world = World::new();
    let profile = LoginProfile {
        username: lodestone_testsupport::unique_username(),
        uuid: uuid::Uuid::new_v4(),
    };
    let server = ServerAddress { host: "127.0.0.1".to_owned(), port: 25604 };
    let mut state = ConnectionState::Handshaking;
    for directive in adapter.begin_login(&profile, &server).unwrap() {
        apply(&mut conn, &mut state, directive).await;
    }
    let mut output = "# Complete Configuration registries, features and tags captured from Minecraft 1.21.11 (774).\n".to_owned();
        ...
        if state == ConnectionState::Configuration {
            ...
            if [7, 12, 13].contains(&id) {
                ...
                if id == 7 {
                    ...
                    let registry = lodestone_v1_21_11::packets::configuration::RegistryData::decode(
                        &mut reader, lodestone_core::Ctx { version: 774 },
