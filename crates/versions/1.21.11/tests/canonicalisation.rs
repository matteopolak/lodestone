...
//! 26.2 block-state id table (`src/generated/canonical.rs` and its deterministic
//! state-id shards), plus hermetic,
...
/// Directory under `src/generated/` containing the contiguous table shards.
const SHARDS: &str = "canonical";
/// Number of contiguous source state ids emitted into each shard.
const SHARD_SIZE: usize = 1_024;
/// Prefix used for generated shard filenames.
const SHARD_PREFIX: &str = "state_to_canonical_";
...
/// All generated files for the table. Keeping the root and shards together
/// means drift checking can reject a partial or stale shard set before any
/// regeneration writes a file.
...
fn generate(states: &[SourceState], reverse: &ReverseIndex) -> GeneratedTable {
    let air_id = *reverse
    ...
    let _ = writeln!(root, "pub const AIR_STATE_ID: u32 = {air_id};\n");
    let mut shards = Vec::with_capacity(mapped.len().div_ceil(SHARD_SIZE));
    for (shard_index, values) in mapped.chunks(SHARD_SIZE).enumerate() {
        let first_id = shard_index * SHARD_SIZE;
        let last_id = first_id + values.len() - 1;
        let filename = format!("{SHARD_PREFIX}{shard_index:05}.in");
        let mut shard = String::new();
        let _ = writeln!(
            shard,
            "// @generated contiguous source state ids {first_id}..={last_id}; do not edit"
        );
        shard.push_str("[\n");
        for id in values {
            let _ = writeln!(shard, "    {id},");
        }
        shard.push_str("]\n");
        shards.push((filename, shard));
        let _ = writeln!(
            root,
            "const SHARD_{shard_index:05}: [u32; {}] = include!(\"canonical/{SHARD_PREFIX}{shard_index:05}.in\");",
            values.len()
        );
    }
    root.push_str(
...
"#,
    ...
    for (shard_index, values) in mapped.chunks(SHARD_SIZE).enumerate() {
        let _ = writeln!(
            root,
            "    copy_shard(&mut table, {}, &SHARD_{shard_index:05});",
            shard_index * SHARD_SIZE
        );
        debug_assert_eq!(values.len(), shards[shard_index].1.lines().count() - 3);
    }
    root.push_str(
        r#"    table
}
...
"#,
    ...
    GeneratedTable { root, shards }
}

/// Returns the exact generated filenames present in `directory`, rejecting
/// an absent directory, an omitted expected shard, or an unexpected stale
/// shard before callers compare any content.
fn committed_shard_names(directory: &std::path::Path, expected: &[String]) -> Vec<String> {
    let entries = std::fs::read_dir(directory)
        .unwrap_or_else(|error| panic!("generated shard directory {} is unreadable: {error}", directory.display()));
    let mut actual = Vec::new();
    for entry in entries {
        let entry = entry.expect("read generated shard directory entry");
        let path = entry.path();
        if path.is_file()
            && path
                .file_name()
                .and_then(|name| name.to_str())
                .is_some_and(|name| name.starts_with(SHARD_PREFIX) && name.ends_with(".in"))
        {
            actual.push(
                path.file_name()
                    .expect("generated shard has a filename")
                    .to_str()
                    .expect("generated shard filename is utf-8")
                    .to_owned(),
            );
        }
    }
    ...
    assert_eq!(
        &actual, expected,
        "generated canonical shard set is missing or stale; regenerate with LODESTONE_REGEN=1"
    );
    actual
}

/// Compares every generated output as one unit. All reads happen before any
/// regeneration write, so a partial checkout can only fail the drift guard;
/// it cannot be mistaken for a valid table.
fn assert_generated_matches(path: &std::path::Path, expected: &GeneratedTable) {
    let committed = std::fs::read_to_string(path).expect("committed table present");
    assert_eq!(
        committed, expected.root,
        "{COMMITTED} is stale vs tests/support/{DUMP_FILE} or the 26.2 registry; regenerate with LODESTONE_REGEN=1 (see the test module docs)"
    );
    let shard_dir = path
        .parent()
        .expect("generated table has a parent")
        .join(SHARDS);
    let expected_names: Vec<String> = expected
        ...
        .collect();
    committed_shard_names(&shard_dir, &expected_names);
    for (name, source) in &expected.shards {
        let shard = std::fs::read_to_string(shard_dir.join(name))
            .unwrap_or_else(|error| panic!("generated canonical shard {name} is unreadable: {error}"));
        assert_eq!(
            shard, *source,
            "generated canonical shard {name} is stale; regenerate with LODESTONE_REGEN=1"
        );
    }
}

/// Writes all generated files through sibling temporary files. The full
/// output is rendered and validated before the first rename, and each shard is
/// renamed only after its complete contents are on disk.
fn write_generated(path: &std::path::Path, generated: &GeneratedTable) {
    let shard_dir = path
        .parent()
        .expect("generated table has a parent")
        .join(SHARDS);
    std::fs::create_dir_all(&shard_dir).expect("create generated canonical shard directory");
    let nonce = format!("{}.{}", std::process::id(), std::thread::current().name().unwrap_or("test"));
    let root_tmp = path.with_extension(format!("rs.tmp-{nonce}"));
    assert!(!root_tmp.exists(), "temporary generated root already exists");
    std::fs::write(&root_tmp, &generated.root).expect("write temporary generated root");

    let mut temporary_shards = Vec::with_capacity(generated.shards.len());
    for (name, source) in &generated.shards {
        let target = shard_dir.join(name);
        let temporary = target.with_extension(format!("in.tmp-{nonce}"));
        assert!(!temporary.exists(), "temporary generated shard already exists");
        std::fs::write(&temporary, source).expect("write temporary generated shard");
        temporary_shards.push((temporary, target));
    }

    for (temporary, target) in temporary_shards {
        std::fs::rename(temporary, target).expect("publish generated canonical shard");
    }
    std::fs::rename(root_tmp, path).expect("publish generated canonical root");
}
...
fn committed_table_matches_dump() {
    ...
    if std::env::var_os("LODESTONE_REGEN").is_some() {
        ...
        eprintln!(
            "regenerated {} plus {} contiguous shards ({} states)",
            path.display(),
            generated.shards.len(),
            states.len()
        );
        return;
    }

    assert_generated_matches(&path, &generated);
}

/// The root remains one flat static array at runtime, while the committed
/// source is split into fixed-size, contiguous state-id shards. This cheap
/// guard catches a missing include, an out-of-order include, or a shard whose
/// values no longer line up with the compiled table without rebuilding the
/// expensive canonical reverse index.
#[test]
fn committed_canonical_shards_are_contiguous_and_compile_into_one_table() {
    let generated = manifest_dir().join(COMMITTED);
    let shard_dir = generated
        .parent()
        .expect("generated table has a parent")
        .join(SHARDS);
    let shard_count = STATES.div_ceil(SHARD_SIZE);
    let expected_names: Vec<String> = (0..shard_count)
        .map(|index| format!("{SHARD_PREFIX}{index:05}.in"))
        .collect();
    let names = committed_shard_names(&shard_dir, &expected_names);
    assert_eq!(names.len(), shard_count);

    let table = &lodestone_v1_21_11::generated_canonical::STATE_TO_CANONICAL;
    assert_eq!(table.len(), STATES);
    let mut next_state_id = 0usize;
    for name in names {
        let source = std::fs::read_to_string(shard_dir.join(&name))
            .unwrap_or_else(|error| panic!("generated canonical shard {name} is unreadable: {error}"));
        let values: Vec<u32> = source
            ...
            .map(|line| {
                line.trim()
                    .strip_suffix(',')
                    .expect("shard value ends with a comma")
                    .parse::<u32>()
                    .expect("shard value is an integer")
            })
            .collect();
        let expected_len = (STATES - next_state_id).min(SHARD_SIZE);
        assert_eq!(
            values.len(),
            expected_len,
            "{name} must contain exactly the next contiguous state-id range"
        );
        assert_eq!(
            &table[next_state_id..next_state_id + values.len()],
            values.as_slice(),
            "{name} does not compile into the corresponding flat table range"
        );
        next_state_id += values.len();
    }
    assert_eq!(next_state_id, STATES, "shards must cover every source state id");
}
