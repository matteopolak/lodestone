...
#[tokio::test]
async fn joined_protocol_774_chest_moves_a_slot_and_closes_cleanly() {
    ...
    assert_eq!(
        source
            ...
            .and_then(|entity| entity.container_slots()[0].as_ref().map(|item| item.item.to_string())),
        Some("minecraft:diamond".to_owned()),
        "the fixture must provide an item independently of the client decoder",
    );
    let (server, client_io) = IntegratedServer::open_in_memory(protocol, Arc::clone(&source), 0);
    let (mut handle, _) = ClientBuilder::new(
        ServerAddress { host: "memory".to_owned(), port: 0 },
        LoginProfile { username: "ChestFixture774".to_owned(), uuid: uuid::Uuid::new_v4() },
        Box::new(adapter_for(774)),
    )
    .player_loaded_policy(PlayerLoadedPolicy::Manual)
    .connect_with(client_io);
    ...
    handle
        .wait_for_spawn(Duration::from_secs(10))
        .await
        .expect("protocol-774 client must reach Play");
    handle
        .wait_for_chunk(lodestone_client::ChunkPos::new(0, 0), Duration::from_secs(10))
        .await
        .expect("protocol-774 chest chunk must arrive");
    ...
    handle
        ...
        .await
        .expect("the chest opens through the hosted protocol-774 path");

    let opened = handle.open_menu().expect("the chest menu remains open");
    assert_eq!(opened.window_id, 1);
    assert_eq!(opened.menu_type.to_string(), "minecraft:generic_9x3");
    ...
    handle
        ...
        .expect("joined client accepts the hosted container click");
    handle
        .wait_for(Duration::from_secs(10), |client| {
...
impl ChunkSource for FixtureSource {
    fn column(&self, _cx: i32, _cz: i32) -> ChunkColumn {
        let mut column = self.column.lock().expect("fixture column lock poisoned").clone();
