...
//! spawn packet carrying a `varint` type id into this one table. The
//! player-spawn packet that the pre-1.20.2 eras carry does not exist at this
//! protocol: a player arrives through the same spawn packet, with
//! [`PLAYER`]'s own registry id.
//!
//! # Why this table is per era and cannot be borrowed
//!
//! The registry is dense and alphabetical, so any insertion renumbers
//! everything after it. This era's registry holds 157 types against the 130
//! the 1.20.6 era carries, and the additions are spread through the alphabet
//! rather than appended — so almost every shared name sits at a *different* id
//! here. Reading a 774 spawn through a neighbouring era's table would name a
//! real, wrong entity for essentially every id, with no error anywhere.
//! `tests/entity_types.rs` asserts that shift from the committed dump rather
//! than trusting this paragraph.
//!
//! This version-specific data never lives in a shared crate; it comes from the
//! jar's own registry report.

use crate::generated_entity_types;

/// Canonical identifier for a player entity.
...
pub struct EntityTypeTable {
    ...
    /// field so the two can disagree and be caught: a hand edit to one half of
    /// a generated file is exactly the drift the generate-or-assert gate
...
/// The era's unified entity registry, rendered from the jar's own registry
/// report — see the module docs for why it is per era.
