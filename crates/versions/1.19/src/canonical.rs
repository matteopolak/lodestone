//! Resolves a flat block-state id from this era's protocol (762) to the
...
//! # Why this exists
//!
//! 1.19.4 is post-Flattening: a chunk-section palette entry is already a
//! single flat state id, not the pre-1.13 `(blockId << 4) | meta` composite
//! the 1.8 and 1.9 eras bridge through `lodestone_canonical::canonical`. But
//! each release's flat id space is **its own** — the game inserts new blocks
//! into the global palette as they are added, so a given block's numeric id
//! drifts release to release. Storing a wire id straight into
//! `lodestone-world` storage unmapped silently renders as whichever unrelated
//! 26.2 block happens to share that number today — no error, no panic, just
//! the wrong terrain. This era's palette holds 23,725 states against the era
//! below's 20,342, so essentially every id above the first few thousand names
//! a different block in the two.
//!
//! # How it works
//!
...
//! bridging it applies, and its data provenance (the jar's own data generator
//! run against the real `.cache/mc/1.19.4/server.jar`).
//!
...
/// docs for why it cannot be shared with any neighbouring era's.
...
fn canonical_to_wire() -> &'static HashMap<u32, ReverseState> {
    CANONICAL_TO_WIRE.get_or_init(|| {
        ...
        for (wire, &canonical) in generated_canonical::STATE_TO_CANONICAL.iter().enumerate() {
            let wire = u32::try_from(wire).expect("protocol-762 wire state fits in u32");
...
/// Returns the unique protocol-762 state for a canonical state.
///
...
pub(crate) fn wire_state_for_762(canonical: u32) -> Option<u32> {
