//! Independent protocol-762 entity fixtures through production ECS ingest.
//!
...
use lodestone_v1_19::packet_ids::play::clientbound;
use lodestone_v1_19::{PROTOCOL_1_19_4, V762Adapter};
use lodestone_world::World;

const ENTITY_ID: i32 = 73;

/// A literal protocol-762 `spawn_entity` body for a pig. Protocol 762 folded
/// living-entity spawns into this packet and adds a head-yaw byte before object
/// data; the non-default fields make either omission observable.
const SPAWN_ENTITY: &[u8] = &[
    0x49, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee,
    0xff, 0x48, 0x40, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x40, 0x51, 0x80, 0x00, 0x00, 0x00,
    0x00, 0x00, 0xc0, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf6, 0x14, 0x1e, 0x00, 0x01, 0x2c,
    0xff, 0x38, 0x00, 0x64,
];
...
const ENTITY_EQUIPMENT: &[u8] = &[0x49, 0x80, 0x00, 0x05, 0x01, 0xb4, 0x06, 0x02, 0x00];
...
fn app_for(metadata: &[u8], equipment: &[u8], attributes: &[u8]) -> App {
    let adapter = V762Adapter::for_protocol(PROTOCOL_1_19_4);
    ...
    for (packet_id, body) in [
        ...
        let directives = adapter
            ...
            .expect("literal protocol-762 body decodes");
        events.extend(
            directives
                .into_iter()
...
#[test]
fn literal_protocol_762_metadata_equipment_and_attributes_reach_ecs() {
    ...
    assert_eq!(
        ...
        "lodestone:legacy_modifier_00000000000000000000000000000001"
    );
...
#[test]
fn literal_protocol_762_wrong_values_change_the_observed_components() {
