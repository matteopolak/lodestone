//! Independent protocol-756 death notification fixtures.
//!
//! The split death-combat body is written directly from its wire shape:
...
use lodestone_v1_17::{adapter_for, packet_ids, PROTOCOL_1_17_1};
...
fn death_combat_event_emits_the_model_death_event() {
    ...
    // message. The fields distinguish VarInt/i32/string decoding and the
    // dispatch id independently from any local encoder.
    let body = hex(
    ...
    let directives = adapter_for(PROTOCOL_1_17_1)
        .handle_packet(
            &mut World::new(),
        ...
        .expect("literal protocol-756 death fixture decodes");
...
fn death_combat_event_rejects_trailing_bytes() {
    ...
    body.push(0xff);
    let error = adapter_for(PROTOCOL_1_17_1)
        .handle_packet(
            &mut World::new(),
            ConnectionState::Play,
            packet_ids::play::clientbound::DEATH_COMBAT_EVENT,
