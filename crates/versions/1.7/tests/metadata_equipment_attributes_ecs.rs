//! Independent protocol-5 entity fixtures through the production ECS ingest.
//!
...
use lodestone_v1_7::packet_ids::play::clientbound;
...
/// Entity 42, shared flags `0x05`, and the metadata-list terminator.
const ENTITY_METADATA: &[u8] = &[0x00, 0x00, 0x00, 0x2a, 0x00, 0x05, 0x7f];

/// Entity 42's helmet slot with three protocol-5 stone items. The slot uses
/// the era's signed item id, count, damage and absent-compressed-NBT fields.
const ENTITY_EQUIPMENT: &[u8] = &[
    0x00, 0x00, 0x00, 0x2a,
    0x00, 0x04,
    0x00, 0x01, 0x03, 0x00, 0x00, 0xff, 0xff,
];
...
fn decode_events(adapter: &V5Adapter, packets: &[(i32, &[u8])]) -> Vec<ClientEvent> {
        ...
        .flat_map(|(packet_id, payload)| {
            adapter
                ...
                .unwrap_or_else(|error| {
                    panic!("literal protocol-5 packet {packet_id} must decode: {error}")
                })
                .into_iter()
...
#[test]
fn literal_protocol_5_metadata_equipment_and_attributes_reach_ecs() {
...
#[test]
fn literal_protocol_5_wrong_values_change_observed_components() {
    ...
    assert_ne!(entity_components(&app).get::<EntityFlags>().map(|flags| flags.0), Some(0x05));
...
#[test]
fn literal_protocol_5_truncated_spawn_is_rejected_before_ingest() {
    let result = V5Adapter::new().handle_packet(
        &mut World::new(),
        ConnectionState::Play,
