//! In-memory integration coverage for the hosted protocol-5 family.
...
#[tokio::test]
async fn position_look_recenters_the_hosted_protocol_5_view() {
    let protocol = lodestone_registry::server_protocol_for_protocol(PROTOCOL)
        .expect("protocol 5 must resolve to a hosted family");
    let source = Arc::new(LegacyFixtureSource::new());
...
fn a_non_hosted_legacy_protocol_is_rejected_before_connection_setup() {
    assert!(lodestone_registry::server_protocol_for_protocol(4).is_none());
    assert!(lodestone_registry::server_protocol_for_protocol(6).is_none());
}
