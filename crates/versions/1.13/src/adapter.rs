...
/// The only protocol this family speaks (Minecraft 1.13.2).
///
/// Note the folder name is `1.13` and this protocol is **404**. Never derive
/// one from the other — ask [`PROTOCOLS`].
pub const PROTOCOL: i32 = PROTOCOL_1_13_2;

/// Protocol version of Minecraft 1.13.2 — this era's only release.
///
/// Read off the server's own status response rather than a table: a 1.13.2
/// server started by `scripts/live-oracles/legacy.sh 1.13.2` answers a
/// server-list ping with `{"name":"1.13.2","protocol":404}`.
...
///
/// **One entry, deliberately.** Every other legacy family in this repo is an
/// era crate covering several releases; 1.13.2 covers one because the wire
/// breaks on both sides of it. Measured from `minecraft-data` with named
/// types inlined and primitive aliases kept, 1.13.2 agrees with 1.12.2 on 104
/// of 125 shared packet shapes and with 1.14.4 on 114 of 142, which is 72%
/// and 73% once each side's additions are counted — both below the ~88% that
/// makes a run of releases one era. Folding it into either neighbour would
/// put a real discontinuity inside a single crate.
pub const PROTOCOLS: &[i32] = &[PROTOCOL_1_13_2];

/// The packet ids this protocol assigns to the packets this adapter names.
///
/// The generated `packet_ids` table is its own module, so a
/// `self.ids().block_dig` path can only ever mean *one* protocol's id. This
/// struct is the indirection an era crate uses to serve several protocols
/// from one adapter body; this era has one member, and the indirection is
/// kept rather than inlined because it is what makes adding a second a table
/// and an `ids_for` arm instead of an edit to every send site. Nothing in
/// this file may name a generated module directly outside `packet_ids_from!`.
#[derive(Debug)]
struct PacketIds {
    ...
    window_click: i32,
    /// `minecraft:crafting_book_data`, serverbound play — the packet that
    /// toggles a recipe-book pane. 1.16 split it into `recipe_book` (the
    /// pane state) and `displayed_recipe`; 404 still has the single packet
    /// with a leading action selector.
    crafting_book_data: i32,
}
...
macro_rules! packet_ids_from {
    ($table:ident) => {
        PacketIds {
            ...
            crafting_book_data: crate::$table::play::serverbound::CRAFTING_BOOK_DATA,
        }
    };
}

/// Minecraft 1.13.2's ids.
static IDS_1_13_2: PacketIds = packet_ids_from!(packet_ids);
...
fn resolve_menu_type(inventory_type: &str, slot_count: u8) -> ResourceKey {
    ...
    key.parse().unwrap_or_else(|_| {
        generic_rows().parse().expect("generic menu key is valid")
    })
}
...
fn slot_to_item_stack(slot: &Slot) -> Result<Option<ItemStack>, AdapterError> {
    ...
    let name = item_name(*id).ok_or_else(|| {
        AdapterError::Decode(format!("unsupported protocol-404 item id {id}"))
    })?;
    let count = u32::try_from(*count).map_err(|_| {
        AdapterError::Decode(format!("invalid protocol-404 item count {count}"))
    })?;
    if count == 0 {
        return Err(AdapterError::Decode(
            "protocol-404 item slot has a zero count".to_owned(),
        ));
    }
    let key = name.parse().map_err(|_| {
        AdapterError::Decode(format!("item registry entry {name} is not a key"))
    })?;
...
/// Recipe books the client tracks at 404: crafting and furnace. The
/// blast-furnace and smoker books arrived with 1.14, so the model's
/// `RecipeBookType` has four variants and this wire carries two.
const RECIPE_BOOK_COUNT: usize = 2;
...
/// The numeric dimension id of the overworld -- the only one of the three
/// that carries sky light.
const OVERWORLD_DIMENSION: i32 = 0;
...
/// Per-connection state used by 1.13.2's client-side player-position-send tick.
...
/// Version adapter implementing protocol 404.
///
/// Holds a [`ChunkShape`] for the paletted chunk decoder. Here the shape
/// genuinely does depend on the dimension — light still travels inside
/// `map_chunk` at 404, and whether a section carries a trailing sky-light
/// array follows from the dimension the join packet announced — so the field
/// is guarded by a [`Mutex`] and updated on `login`/`respawn`.
#[derive(Debug, Clone)]
pub struct V404Adapter {
    ...
    /// `(open, filtering)` for the crafting and furnace recipe books, in
    /// `RecipeBookType` ordinal order.
    ///
    /// `crafting_book_data` re-states both panes on every change, so a caller
    /// that names one needs the other to keep the value it last had rather
    /// than closing.
    recipe_books: Arc<Mutex<[(bool, bool); RECIPE_BOOK_COUNT]>>,
}

impl Default for V404Adapter {
    fn default() -> Self {
        Self::new()
    }
}
...
impl V404Adapter {
    ...
    /// id table, block-state table, entity registry and chunk shape once,
    /// here.
    ///
    /// # Panics
    ///
    ...
    /// I speaking" reaches the codecs. It is a per-instance value rather than
    /// a constant so that a `#[mc(protocols)]` precondition is checked
    /// against the negotiated number, not a compile-time assumption.
    ...
    /// Selects the 1.13.2 movement shape. This is deliberately local to the
    /// family: 1.13 shares the 1.12 rule, but not 1.8's idle cadence or the
    ...
    /// Selects the chunk shape for a numeric dimension.
    ///
    /// Only the overworld (`0`) carries sky light, and at 404 sky light is
    /// **inside** `map_chunk`: a section is `[blocks][blockLight][skyLight?]`,
    /// so guessing wrong shifts every section after the first by 2,048 bytes
    /// and the chunk buffer's own length check is what turns that into an
    /// error instead of a garbled column. The packet cannot say from its own
    /// bytes which case it is; only the join/respawn packet knows.
    fn set_chunk_shape(&self, dimension: i32) {
        let shape = if dimension == OVERWORLD_DIMENSION {
            ChunkShape::overworld(self.protocol)
        } else {
            ChunkShape::no_skylight(self.protocol)
        };
        if let Ok(mut current) = self.shape.lock() {
            *current = shape;
        }
    }
...
/// This family serves one protocol, so the argument selects the only id
/// table, block-state table, entity-type registry and chunk shape there is —
/// but it is still resolved through the same seam a multi-protocol era uses,
/// so adding a second member is a table and an `ids_for` arm rather than an
/// edit to every send site.
///
/// # Panics
///
...
#[must_use]
pub fn adapter_for(protocol: i32) -> V404Adapter {
    debug_assert!(
        PROTOCOLS.contains(&protocol),
        "adapter_for({protocol}) is outside this family's PROTOCOLS ({PROTOCOLS:?}); \
         callers must test membership before constructing"
    );
    V404Adapter::for_protocol(protocol)
}

/// Maps the model's `RecipeBookType` onto the ordinal 1.13.2's
/// `crafting_book_data` pane list uses. 1.13.2 has the crafting and furnace
/// books only — the blast-furnace and smoker books arrived with 1.14 — so the
/// latter two clamp onto the furnace pane rather than addressing a pane the
/// wire does not have. The ordinal itself has held stable from 1.13 through
/// protocol 776.
fn recipe_book_type_to_ordinal(book_type: RecipeBookType) -> i32 {
    match book_type {
        RecipeBookType::Crafting => 0,
        // The blast-furnace and smoker books were added in 1.14 and have no
        // pane on this wire; they clamp onto the furnace pane, which is the
        // only smelting-shaped one 404 carries.
        RecipeBookType::Furnace | RecipeBookType::BlastFurnace | RecipeBookType::Smoker => 1,
    }
}
...
impl V404Adapter {
    ...
    fn handle_login(&self, packet_id: i32, payload: &[u8]) -> Result<Vec<Directive>, AdapterError> {
        ...
        if packet_id == self.ids().login_success {
            // Validate the profile decodes, then advance. 404 sends the
            // UUID as a dashed string; 1.16 replaced it with sixteen raw
            // bytes. The shared definition's `#[mc(protocols = "47..=578")]`
            // is what stops the wrong one being used here -- reading the
            // binary form off a string body consumes the length prefix and
            // the first fifteen characters as a UUID and then reads the rest
            // as a length-prefixed name, which does not fail, it just
            // produces nonsense.
...
type PlayHandler =
    fn(&V404Adapter, &mut dyn WorldSink, &[u8]) -> Result<Vec<Directive>, AdapterError>;

impl V404Adapter {
    /// `minecraft:login`.
    ///
    /// The dimension is a signed integer here, not a namespaced name, so it
    /// is mapped through [`legacy_dimension_name`] and recorded for the
    /// packets that carry no dimension field at all. It also selects the
    /// chunk shape: only the overworld carries sky light inside `map_chunk`,
    /// and getting that wrong shifts every section after the first by 2,048
    /// bytes.
    fn handle_play_login(
        ...
        adapter.set_chunk_shape(body.dimension);
    ...
    /// `minecraft:map_chunk`. Decodes the paletted 1.13.2 column into
    /// version-free storage and applies it to the world through the sink,
    /// emitting only a lightweight notification. Light travels *inside* this
    /// packet at 404 (1.14 split it out into `update_light`), so the loaded
    /// column arrives fully lit.
    ...
    /// `minecraft:unload_chunk`. 1.13.2 has a dedicated forget packet (two
    /// ints).
    ...
    fn handle_play_spawn_entity(
        ...
        // 404 sends the object type as a signed byte into the same unified
        // registry `spawn_entity_living` indexes with a VarInt.
        ...
        // Velocity is unconditional here, but a stationary object still
    ...
    /// `minecraft:respawn`. A numeric dimension, as in `login`, so it
    /// re-selects the chunk shape for the same reason.
    ...
    fn handle_play_difficulty(
        ...
        Ok(vec![Directive::Emit(ClientEvent::DifficultyChanged {
            difficulty,
            // 404's packet carries no lock flag at all -- 1.14 added it when
            // difficulty became client-settable -- so this reports the
    ...
    /// `minecraft:open_window` with the protocol-404 string type and slot
    /// count. The title is already a JSON component from the server.
    ...
    /// `minecraft:window_items` provides the authoritative full menu snapshot.
    ...
    /// `minecraft:set_slot` updates either the cursor, player inventory, or
    /// the currently open menu according to its signed window sentinel.
...
static CLIENTBOUND: &[(&str, lodestone_core::dispatch::Handler<PlayHandler>)] = &[
    ...
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_map_chunk,
        ),
    ),
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_unload_chunk,
        ),
    ),
    (
        "minecraft:keep_alive",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_keep_alive,
        ),
    ),
    (
        "minecraft:chat",
    ...
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_position,
        ),
    ),
    (
        "minecraft:spawn_entity_living",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_spawn_entity_living,
        ),
    ),
    (
        "minecraft:spawn_entity",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_spawn_entity,
        ),
    ),
    (
        "minecraft:named_entity_spawn",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_named_entity_spawn,
        ),
    ),
    (
        "minecraft:rel_entity_move",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_rel_entity_move,
        ),
    ),
    (
        "minecraft:entity_look",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_entity_look,
        ),
    ),
    (
        "minecraft:entity_move_look",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_entity_move_look,
        ),
    ),
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_entity_teleport,
        ),
    ),
    (
        "minecraft:entity_velocity",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_entity_velocity,
        ),
    ),
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_entity_destroy,
        ),
    ),
    (
        "minecraft:kick_disconnect",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_kick_disconnect,
        ),
    ),
    (
        "minecraft:update_health",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_update_health,
        ),
    ),
    (
        "minecraft:respawn",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_respawn,
        ),
    ),
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_spawn_position,
        ),
    ),
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_entity_status,
        ),
    ),
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_entity_head_rotation,
        ),
    ),
    (
        "minecraft:animation",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_animation,
        ),
    ),
    (
        "minecraft:abilities",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_abilities,
        ),
    ),
    (
        "minecraft:difficulty",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_difficulty,
        ),
    ),
    (
        "minecraft:update_time",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_update_time,
        ),
    ),
    (
        "minecraft:playerlist_header",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_playerlist_header,
        ),
    ),
    (
        "minecraft:attach_entity",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_attach_entity,
        ),
    ),
    (
        "minecraft:set_passengers",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_set_passengers,
        ),
    ),
    (
        "minecraft:collect",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_collect,
        ),
    ),
    (
        "minecraft:entity_effect",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_entity_effect,
        ),
    ),
    (
        "minecraft:remove_entity_effect",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_remove_entity_effect,
        ),
    ),
    (
        "minecraft:spawn_entity_experience_orb",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_spawn_entity_experience_orb,
        ),
    ),
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_block_change,
        ),
    ),
    (
    ...
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_experience,
        ),
    ),
    (
        "minecraft:vehicle_move",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_vehicle_move,
        ),
    ),
    (
        "minecraft:select_advancement_tab",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_select_advancement_tab,
        ),
    ),
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_open_sign_entity,
        ),
    ),
    (
        "minecraft:camera",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_camera,
        ),
    ),
    (
        "minecraft:held_item_slot",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_held_item_slot,
        ),
    ),
    (
        "minecraft:close_window",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_close_window,
        ),
    ),
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_open_window,
        ),
    ),
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_window_items,
        ),
    ),
    (
        "minecraft:set_slot",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_set_slot,
        ),
    ),
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_craft_progress_bar,
        ),
    ),
    (
        "minecraft:title",
    ...
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_tab_complete,
        ),
    ),
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_player_info,
        ),
    ),
    (
        "minecraft:boss_bar",
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_boss_bar,
        ),
    ),
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_combat_event,
        ),
    ),
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_world_border,
        ),
    ),
    (
        "minecraft:teams",
    ...
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_scoreboard_display_objective,
        ),
    ),
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_scoreboard_objective,
        ),
    ),
    (
        ...
        lodestone_core::dispatch::Handler::new(
            lodestone_core::ProtocolRange::ALL,
            V404Adapter::handle_play_scoreboard_score,
        ),
    ),
];
...
/// which is the anti-island guard that replaces an if-chain's trailing
/// `Ok(Vec::new())`. Derived by diffing this crate's handler list against the
/// 86-entry 404 `ENTRIES` table, then checking `crates/versions/26.2/src/adapter/`
/// for each gap by canonical name or an obvious Mojang-naming synonym (v26-2
/// uses Mojang's own names, which differ from minecraft-data's for the same
/// packet). Two entries name no later packet at all, and say so.
static IGNORED: &[lodestone_core::dispatch::IGNORED] = &[
    ...
    // 1.16 folded the lightning bolt into the generic `spawn_entity` table;
    // 404 still has its own packet for it.
    lodestone_core::dispatch::IGNORED::new(
        "minecraft:spawn_entity_weather",
        "v26-2 has this; backport (ADD_ENTITY, lightning)",
    ),
    // Removed in 1.14, which replaced the bed-occupancy packet with a `pose`
    // entity-metadata value. There is no v26-2 packet to backport from: the
    // information moved into metadata, which this crate decodes but does not
    // yet translate into a canonical event.
    lodestone_core::dispatch::IGNORED::new(
        "minecraft:bed",
        "removed in 1.14; sleep state became an entity-metadata pose, with no packet of its own \
         in any later protocol to backport",
    ),
];
...
impl V404Adapter {
    ...
    /// [`ids_for`] resolves an id table. One slot here because this era has
    /// one member; the array is kept rather than collapsed to a bare
    /// `OnceLock` so that adding a second protocol is a slot and an `ids_for`
    /// arm, not a restructuring.
    ///
    /// # Panics
    ///
    ...
    fn play_dispatch_table(&self) -> &'static lodestone_core::dispatch::Table<'static, PlayHandler> {
        ...
        TABLES[slot].get_or_init(|| {
            ...
            .unwrap_or_else(|err| {
                panic!(
                    "v1-13 play dispatch table for protocol {} must build: every clientbound \
...
impl VersionAdapter for V404Adapter {
    ...
    fn encode_action(
        ...
        match action {
            ...
            // Placing a block / using an item on a block. 404 sends the
            // packed position first, then a varint face, a varint hand and a
            // float cursor -- 1.14 moved the hand to the front and appended
            // `inside_block`. No inline item (1.13 dropped it; the server
            // resolves it) and no block-prediction `sequence` (added 1.19),
            // both dropped deliberately. The model's `inside_block` has no
            // 404 field and is dropped with it.
            ...
            // Using an item in the air is the dedicated `use_item` packet
            // from 1.9 on (the 1.8 (-1,-1,-1) `block_place` sentinel no
            // longer works). The model's `rotation` and `sequence` have no
            // 404 equivalent and are dropped.
            ...
            // (attack has no hand); the `sneaking` flag 1.16 appended is
            // dropped by its own `since` predicate here. Each mouse value is
            ...
            // Container clicks use this era's action counter instead of a
            // modern state id. The shared state id supplies that counter and
            ...
            ClientAction::SetFlying { flying } => {
                let body = PlayerAbilities {
                    ...
                    // the server ignores on this packet. 1.16 dropped both
                    // fields; 404 still sends them.
            ...
            // 1.13.2 has two recipe books, not four, and re-states both on
            // every change -- so the pane the caller did not name is filled
            // from the adapter's own record rather than defaulted shut. The
            // blast-furnace and smoker books do not exist on this wire;
            // `recipe_book_type_to_ordinal` clamps them onto the furnace pane
            // rather than addressing a pane the packet has no field for.
...
mod mob_effect_tests {
    ...
    fn encoded_update(adapter: &V404Adapter, wire_id: i8) -> Vec<u8> {
        adapter
    ...
    fn encoded_remove(adapter: &V404Adapter, wire_id: i8) -> Vec<u8> {
        adapter
    ...
    fn packet_ingress_resolves_one_based_speed_and_rejects_unknown_signed_ids() {
        ...
        let applied = V404Adapter::handle_play_entity_effect(
            &adapter,
            &mut world,
        ...
        let removed = V404Adapter::handle_play_remove_entity_effect(
            &adapter,
            &mut world,
        ...
        for wire_id in [
            ...
            let mut world = World::new();
            let error = V404Adapter::handle_play_entity_effect(
                &adapter,
                &mut world,
            ...
            let error = V404Adapter::handle_play_remove_entity_effect(
                &adapter,
                &mut world,
...
mod dispatch_coverage_tests {
    ...
    /// for every protocol in this era, must construct successfully --
    ...
    /// not by a name lookup that would agree with any table.
    ///
    /// 404's `update_health` is id 68 and its `keep_alive` is 33; the era
    /// above puts `update_health` at 72 and `keep_alive` at 32, and the era
    /// below puts them at 65 and 31. The pair separates all three, which
    /// neither packet does alone -- and the values here come from the
    /// generated table, so this asserts the adapter reads that table rather
    /// than asserting the table's contents twice.
    #[test]
    fn the_table_uses_404s_own_ids() {
        let ids = |name: &str| {
            ids_for(PROTOCOL_1_13_2)
                .play_clientbound_entries
                .iter()
                .find(|(entry, _)| *entry == name)
                .map(|(_, id)| *id)
                .expect("404 carries both probes")
        };
        assert_eq!(
            (
                ids("minecraft:update_health"),
                ids("minecraft:keep_alive")
            ),
            (0x44, 0x21)
        );
    }

    /// Negative control: drop the `minecraft:tags` `IGNORED` entry from a
    /// local copy of the list. Its packet id then has neither a bound
    /// handler nor a matching ignore entry -- exactly the `_ =>` island this
    /// table exists to catch -- so construction must fail, and by name,
    /// proving the detector actually works rather than just trusting the
    ...
    fn negative_control_dropping_one_ignored_entry_fails_construction() {
        ...
        let position = ignored_missing_one
            .iter()
            .position(|entry| entry.name == "minecraft:tags")
            .expect("the IGNORED list names minecraft:tags");
        ...
        let tags_id = entries
            ...
            .map(|(_, id)| *id)
            .expect("404 carries minecraft:tags");
        let table = lodestone_core::dispatch::Table::build(
            PROTOCOL,
            entries,
            CLIENTBOUND,
