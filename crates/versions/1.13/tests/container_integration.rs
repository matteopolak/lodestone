//! Protocol-404 container flow through the live integrated server.
...
fn profile() -> LoginProfile {
    LoginProfile {
        username: "Protocol404Chest".to_owned(),
...
#[tokio::test]
async fn protocol_404_chest_move_reaches_the_live_inventory_consumer() {
    let protocol = lodestone_registry::server_protocol_for_protocol(404)
        .expect("protocol 404 must resolve to a hosted server protocol");
    ...
    handle
        ...
        .await
        .expect("protocol-404 client must reach Play");
    handle
    ...
    for (slot, expected) in [(0, None), (27, Some(1))] {
        handle
            ...
            .expect("protocol-404 client must send the pickup click");
    ...
    handle
        ...
        .expect("protocol-404 client must close the chest window");
    handle.shutdown();
