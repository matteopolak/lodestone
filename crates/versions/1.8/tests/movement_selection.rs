//! Selection tests for 1.8.9's `EntityPlayerSP.onUpdateWalkingPlayer`.
...
fn encode(adapter: &V47Adapter, action: ClientAction) -> Option<(i32, Vec<u8>)> {
    adapter
        .encode_action(ConnectionState::Play, &action)
        .expect("movement encoding")
}

fn position(x: f64, y: f64, z: f64, on_ground: bool) -> Vec<u8> {
    let mut bytes = Vec::new();
    bytes.extend_from_slice(&x.to_be_bytes());
    bytes.extend_from_slice(&y.to_be_bytes());
    bytes.extend_from_slice(&z.to_be_bytes());
    bytes.push(u8::from(on_ground));
    bytes
}
...
#[test]
fn vanilla_1_8_uses_position_look_position_and_look_by_dirty_axes() {
...
#[test]
fn vanilla_1_8_sends_flying_every_idle_tick_and_positions_on_the_21st() {
    let adapter = V47Adapter::new();
    ...
    for _ in 0..20 {
        assert_eq!(
            encode(&adapter, action(BASE_POS, BASE_ROT, true)),
            Some((play::serverbound::FLYING, vec![1]))
        );
    }
    assert_eq!(
        encode(&adapter, action(BASE_POS, BASE_ROT, true)),
        Some((
            play::serverbound::POSITION,
            position(BASE_POS.x, BASE_POS.y, BASE_POS.z, true)
        ))
    );
}

#[test]
fn vanilla_1_8_keeps_sub_threshold_jitter_out_of_position_packets() {
    let adapter = V47Adapter::new();
    encode(&adapter, action(BASE_POS, BASE_ROT, true));
    let jitter = Vec3 { x: BASE_POS.x + 0.02, ..BASE_POS };
    assert_eq!(
        encode(&adapter, action(jitter, BASE_ROT, true)),
        Some((play::serverbound::FLYING, vec![1]))
    );
}

#[test]
fn vanilla_1_8_uses_a_strict_nine_e_minus_four_position_boundary() {
    let adapter = V47Adapter::new();
    let zero = Vec3::default();
    let rotation = Rotation::default();
    assert_eq!(encode(&adapter, action(zero, rotation, false)), Some((play::serverbound::FLYING, vec![0])));

    let exact = Vec3 { x: 0.03, ..zero };
    assert_eq!(exact.x * exact.x, 9e-4);
    assert_eq!(encode(&adapter, action(exact, rotation, false)), Some((play::serverbound::FLYING, vec![0])));
...
#[test]
fn fresh_1_8_adapter_starts_from_a_zero_baseline_and_clones_share_it() {
    let adapter = V47Adapter::new();
    let zero = Vec3::default();
    let rotation = Rotation::default();
    let (id, body) = encode(&adapter, action(zero, rotation, false)).expect("fresh base flying");
    ...
    assert_eq!(
        encode(&clone, action(moved, rotation, false)),
        Some((play::serverbound::FLYING, vec![0]))
    );
}
