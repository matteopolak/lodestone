//! Seam tests for protocol 47's window/inventory and player-list packets.
//!
//! `OpenWindow`/`CloseWindow`/`SetSlot`/`WindowItems` already had correct,
//! tested wire codecs (`tests/inventory.rs`) that `V47Adapter::handle_packet`
...
use lodestone_v1_8::packets::player_info::{PlayerInfoAction, PlayerInfoEntry};
use lodestone_v1_8::packets::slot::Slot;
use lodestone_v1_8::packets::window::{CloseWindow, OpenWindow, SetSlot, WindowItems};
...
fn window_items_resolves_known_items_and_treats_unknown_ids_as_empty() {
    let payload = encode(&WindowItems {
        ...
        items: vec![
            ...
            Slot::Empty,        // empty
            a_stack(9999, 1),   // no such 1.8 item id
    ...
    match dispatch(play::clientbound::WINDOW_ITEMS, &payload).as_slice() {
        [
            ...
            assert!(carried_item.is_none(), "1.8's window_items has no cursor field");
        }
...
fn player_info_add_player_carries_gamemode_ping_name_and_skin_properties() {
    ...
    let entries = vec![PlayerInfoEntry {
        ...
        action: PlayerInfoAction::AddPlayer {
            name: "Notch".into(),
            properties: vec![lodestone_v1_8::packets::player_info::PlayerInfoProperty {
                name: "textures".into(),
