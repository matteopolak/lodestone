//! Hosted protocol-47 container flow through the integrated server.
...
impl FixtureSource {
    fn new() -> Self {
        ...
        column.set_block_id(TARGET.x, TARGET.y, TARGET.z, Block::Dandelion.default_state());
        column.set_block_id(CHEST.x, CHEST.y, CHEST.z, Block::Chest.default_state());
        Self {
...
#[tokio::test]
async fn protocol_47_chest_move_reaches_the_live_inventory_consumer() {
    let protocol = lodestone_registry::server_protocol_for_protocol(47)
        .expect("protocol 47 must resolve to a hosted server protocol");
    ...
    let (mut handle, mut events) = ClientBuilder::new(
        ...
        LoginProfile {
            username: "Protocol47Chest".to_owned(),
    ...
    handle
        ...
        .await
        .expect("protocol-47 client must reach Play");
    handle
        ...
        .expect("protocol-47 client must use the chest");
    ...
    for (slot, expected) in [(0, None), (27, Some(1))] {
        handle
            ...
            .expect("protocol-47 client must send the pickup click");
    ...
    handle
        ...
        .expect("protocol-47 client must close the chest window");
    handle.shutdown();
