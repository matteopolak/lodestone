...
// `ClientboundChat` is byte-identical to v1-9's (measured), shared via
...
// `SpawnPosition` is byte-identical to v1-9's (measured), shared via
...
// ServerboundFlying are byte-identical across v1-8/v1-9/v1-14 (measured; raw
...
// `BlockDig` is byte-identical to v1-9's (measured), shared via
// `lodestone-protocol-common` -- see `packets::position`'s module docs. Not
// shared with v1-14: its `Position` field type has an incompatible 1.14+ bit
// layout.
