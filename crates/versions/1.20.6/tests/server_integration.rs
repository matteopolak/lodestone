...
fn adapter_block_use_reaches_the_registry_selected_host_consumer() {
    ...
    let Some((packet_id, payload)) = adapter
        ...
        .expect("the protocol-766 adapter must encode a block use")
    else {
...
async fn registry_selected_protocol_766_reaches_play_and_confirms_a_block_break() {
    ...
    let (mut handle, _) = ClientBuilder::new(address, profile, Box::new(adapter_for(766)))
    ...
    handle
        ...
        .await
        .expect("protocol-766 login reaches Play");
    handle
        .wait_for_chunk(lodestone_client::ChunkPos::new(0, 0), Duration::from_secs(10))
        .await
        .expect("protocol-766 chunk arrives");
...
async fn initial_and_live_border_light_use_the_open_east_column() {
    ...
    assert!(lodestone_registry::server_protocol_for_protocol(766)
        .expect("protocol 766 must resolve to the hosted family")
