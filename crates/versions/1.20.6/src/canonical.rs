//! Resolves a flat block-state id from this era's protocol (766) to the
...
//! # Why this exists
//!
//! 766 is post-Flattening: a chunk-section palette entry is already a single
//! flat state id, not the pre-1.13 `(blockId << 4) | meta` composite the 1.8
//! and 1.9 eras bridge through `lodestone_canonical::canonical`. But each
//! release's flat id space is **its own** — the game inserts new blocks into
//! the global palette as they are added, so a given block's numeric id drifts
//! release to release. Storing a wire id straight into `lodestone-world`
//! storage unmapped silently renders as whichever unrelated 26.2 block
//! happens to share that number today: no error, no panic, just the wrong
//! terrain. This era's palette holds 26,684 states (counted from the jar's
//! own block report) against 26.2's own count, so essentially every id above
//! the first few thousand names a different block in the two.
//!
//! # How it works
//!
...
//! run against the real `.cache/mc/1.20.6/server.jar`).
//!
