...
//! spawn packet carrying a `varint` type id into this one table; the separate
//! mob-spawn packet the pre-1.19 eras carry does not exist at this protocol.
//! The player-spawn packet carries no type at all: it is always [`PLAYER`].
//!
//! # Why this table is per era and cannot be borrowed
//!
//! The registry is dense and alphabetical, so any insertion renumbers
//! everything after it. This era adds six types (`armadillo`, `bogged`,
//! `breeze`, `breeze_wind_charge`, `ominous_item_spawner`, `wind_charge`) to
//! the 124 the 1.19 era carries, and `armadillo` sorts second — so 122 of the
//! 124 shared names sit at a *different* id here. Reading a 766 spawn through
//! a neighbouring era's table would therefore name a real, wrong entity for
//! essentially every id, with no error anywhere. `tests/entity_types.rs`
//! asserts that shift from the committed dump rather than trusting this
//! paragraph.
//!
//! This version-specific data never lives in a shared crate; it comes from
//! the jar's own registry report.

use crate::generated_entity_types;

/// Canonical identifier for a player entity (the player-spawn packet).
...
pub fn table_for(protocol: i32) -> &'static EntityTypeTable {
    match protocol {
        crate::adapter::PROTOCOL_1_20_6 => &TABLE,
        other => panic!(
