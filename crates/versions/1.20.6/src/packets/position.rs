...
//!
//! Two layouts exist and both decode without error: pre-1.14 packs
//! `x(26) | y(12) | z(26)` (y in the middle) and 1.14 onward packs
//! `x(26) | z(26) | y(12)` (y in the low bits). A wrong choice is invisible
//! to a round-trip against our own encoder, so this protocol's order comes
//! from real bytes instead: the world-spawn packet of a flat 1.20.6 world
//! carries `0x0000000000000fc4`, which is `(0, -60, 0)` under the later
//! layout and `(0, 0, 4036)` under the earlier one. The floor of that world
//! is at y = -60 and the player spawned there, so the later layout is the
//! one this era uses. `tests/position.rs` pins those exact bytes.
