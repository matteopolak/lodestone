...
use lodestone_v26_2::packets::common::{ClientInformation, KeepAlive};
use lodestone_v26_2::packets::configuration::{
    ClientboundKnownPacks, KnownPack, ServerboundKnownPacks,
};
use lodestone_v26_2::packets::game::{ChatMessage, GameLogin, MessageSignature};
use lodestone_v26_2::packets::handshake::Intention;
use lodestone_v26_2::packets::login::{
...
fn crate_root_adapter_constructor_matches_new() {
    assert_eq!(
        lodestone_v26_2::adapter().protocol_version(),
...
fn build_encryption_response_frames_the_key_packet() {
    ...
    let directive = adapter()
        ...
        .expect("v26-2 builds the key packet");
...
fn unsupported_action_returns_none() {
    ...
    // TCP socket — so this action can never lower to a packet in v26-2 (or any
