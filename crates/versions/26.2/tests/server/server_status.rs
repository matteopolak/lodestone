...
//!    assertion — vanilla's own server-status record source,
//!    its own clientbound status-response packet source,
//!    its own status/play ping-request and pong-response packet sources, and
//!    vanilla's own server-status packet-listener source for the lifecycle.
...
/// `lodestone_v26_2::PROTOCOL`, for the same reason as `EXPECTED_MOTD` — and
