...
//! `lodestone-v26-2` is `#![forbid(unsafe_code)]`, which `#[allow]` cannot
...
//!
//! ```text
//! cargo test --release -p lodestone-v26-2 --test chunk_encode_cycles -- --ignored --nocapture
//! ```
//!
...
// `lodestone-v26-2`'s own `#![forbid(unsafe_code)]`. `forbid` cannot be overridden
