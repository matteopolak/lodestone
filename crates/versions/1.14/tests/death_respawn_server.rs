//! Literal serverbound death/respawn controls for the three hosted
//! v1.14-era protocols.
//!
...
use lodestone_v1_14::{packet_ids, packet_ids_498, packet_ids_578};

fn assert_respawn(protocol: i32, packet_id: i32) {
    let server = lodestone_registry::server_protocol_for_protocol(protocol)
        .expect("every hosted v1.14-era row must select a server protocol");
    assert_eq!(
...
#[test]
fn hosted_498_578_and_754_accept_literal_perform_respawn() {
    assert_respawn(
        498,
        packet_ids_498::play::serverbound::CLIENT_COMMAND,
    );
    assert_respawn(
        578,
        packet_ids_578::play::serverbound::CLIENT_COMMAND,
    );
    assert_respawn(754, packet_ids::play::serverbound::CLIENT_COMMAND);
}

#[test]
fn client_command_requires_an_exact_body() {
    let server = lodestone_registry::server_protocol_for_protocol(754)
        .expect("protocol 754 must select a server protocol");
    assert_eq!(
        server.decode(
            State::Play,
