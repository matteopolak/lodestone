//! Hosted 1.14-era container flow through the integrated server.

use std::sync::{Arc, Mutex};
use std::time::Duration;

use lodestone_client::{ClientBuilder, EventStream, LoginProfile, PlayerLoadedPolicy, ServerAddress};
use lodestone_model::{BlockFace, BlockPos, ClientAction, ClientEvent, ContainerClickType, ContainerSlotChange, ContainerStateId, Vec3f};
...
async fn assert_protocol(protocol_version: i32) {
    ...
    let profile = LoginProfile { username: format!("Container{protocol_version}"), uuid: uuid::Uuid::new_v4() };
    let address = ServerAddress { host: "memory".to_owned(), port: 0 };
    ...
    handle.wait_for_spawn(Duration::from_secs(20)).await.expect("client must reach Play");
    handle
        ...
        .expect("client must use the chest");
    ...
    for (slot, expected) in [(0, None), (27, Some(1))] {
        handle
            ...
            .expect("client must send pickup click");
    ...
    handle
        ...
        .expect("client must close the chest window");
    handle.shutdown();
...
#[tokio::test]
async fn protocol_498_chest_mutation_is_authoritative() {
    assert_protocol(498).await;
}

#[tokio::test]
async fn protocol_578_chest_mutation_is_authoritative() {
    assert_protocol(578).await;
}

#[tokio::test]
async fn protocol_754_chest_mutation_is_authoritative() {
    assert_protocol(754).await;
}
