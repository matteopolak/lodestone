...
use lodestone_v1_14::packets::entity::SpawnEntityExperienceOrb;
...
use lodestone_v1_14::packets::window::{CloseWindow, HeldItemSlot};
...
fn spawn_position_uses_dimension_from_prior_login() {
    ...
    let payload = encode(&SpawnPosition {
        location: lodestone_v1_14::packets::position::Position::new(10, 64, -5),
    });
...
fn open_sign_entity_dispatches() {
    let payload = encode(&OpenSignEntity {
        location: lodestone_v1_14::packets::position::Position::new(3, 70, 9),
    });
...
fn block_change_writes_a_flattened_state_id_and_reports_relative_coords() {
    ...
    let mut w = Writer::default();
    let packed = lodestone_v1_14::packets::position::pack_position(lodestone_model::BlockPos::new(
        17, 70, -1,
    ));
    w.i64(packed);
...
// length, matches with optional tooltips), so unlike v1-8/v1-9 no
