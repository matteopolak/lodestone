...
//! sending a constant.
//!
//! * **dampening** is vanilla's own "get light dampening" accessor, `0..=15`.
//!   The engine applies `max(1, ·)`
//!   itself — see vanilla's own light-engine "get opacity" step — so this is
//!   the **raw** dampening, not the stepped opacity. Air and glass are `0`; water,
//!   ice and leaves are `1`; a full solid is `15`.
//! * **emission** is vanilla's own "get light emission" accessor, `0..=15`. Non-emitters are
//!   `0`.
//!
...
//! vanilla's own block-properties builder (emission is a private per-state
//! function, dampening a protected method with per-block overrides). It is generated from
