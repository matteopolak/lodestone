...
fn generate(dump: &Dump) -> String {
    ...
    out.push_str(
        "// @generated by `cargo test -p lodestone-data --test shade_brightness -- --ignored`\n\
         ...
         // each block state's own shade-brightness accessor, protocol 776 / Minecraft\n\
         // 26.2).\n\
...
fn exactly_seven_classes_override_shade_brightness() {
    ...
    // and the copper grates' own waterloggable-transparent-block subclass is in it.
    assert_eq!(
...
fn the_divergence_population_matches_vanilla_by_name() {
    ...
    let cases: &[(&str, bool)] = &[
        ...
        // Copper grates' own class is a subclass of the transparent-block
        // family — also 1.0, so they agree with culling too.
