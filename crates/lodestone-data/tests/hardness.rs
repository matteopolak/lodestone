...
fn generate(rows: &[Row]) -> String {
    ...
    out.push_str(
        "// @generated by `cargo test -p lodestone-data --test hardness -- --ignored`\n\
         // from tests/support/hardness_jvm.txt (a headless 26.2 server dump of each\n\
         // block state's own destroy-speed and requires-correct-tool accessors,\n\
...
fn committed_table_matches_the_committed_dump_bit_for_bit() {
    ...
    for row in &rows {
        let state_id = StateId::new(row.id as u32)
