//! The per-block-state **sound-type** census generated into
...
/// One deduplicated sound type: raw float bits plus five sound-event registry
...
fn generate(dump: &Dump) -> String {
    ...
    out.push_str(
        "// @generated by `cargo test -p lodestone-data --test sound_types -- --ignored`\n\
         // from tests/support/sound_types_jvm.txt (a headless 26.2 server dump of each\n\
         // block state's own sound-type accessor, protocol 776 / Minecraft 26.2). DO\n\
         // NOT EDIT BY HAND. Regenerate with LODESTONE_REGEN=1 (see the test module docs).\n",
    );
    out.push_str(
        "//! Generated per-block-state sound-type table for protocol 776 (Minecraft\n\
    ...
    let _ = writeln!(
        out,
        "/// De-duplicated distinct sound-type values ({} of them), as\n\
...
/// The distinct count, asserted so a version bump that adds a sound type fails
...
/// every distinct sound-type object in use has a distinct seven-tuple, so the
...
fn value_dedup_collapses_nothing() {
    ...
    assert_eq!(
        ...
        (126, 126),
        "distinct values vs distinct sound-type objects — if these diverge, the \
...
fn no_state_carries_the_dead_twisting_vines_pitch() {
    ...
    assert_eq!(
        ...
        "the only pitches in use are 1.0 and METAL's 1.5; 0.5 would mean \
         the dead twisting-vines sound type became reachable"
    );
    assert_eq!(
        as_floats(&volumes),
        vec![0.3f32, 1.0f32],
    ...
    for id in vine_ids {
        ...
        assert_eq!(
            ...
            "twisting vines use the weeping-vines sound type, not their own"
        );
    }
}
...
/// Vanilla's own per-state sound-type accessor is `protected` and overridden in exactly one class,
...
fn decorated_pot_is_the_only_per_state_sound_type() {
    ...
    assert_eq!(
        ...
        "the sound-type override set changed"
    );
...
fn the_common_break_sounds_match_vanilla_by_name() {
    ...
    let cases: &[(&str, &str, &str)] = &[
        ...
        // Air has a sound type too (stone's) — the gotcha a caller must guard.
