...
//!
//! ```text
//! LODESTONE_REGEN=1 cargo test -p lodestone-v770 --test entity_census \
...
use lodestone_v770::entity_census::{pushes_players, TYPE_COUNT};
...
fn generate(rows: &[Row]) -> String {
    ...
    out.push_str(
        "// @generated by `cargo test -p lodestone-v770 --test entity_census -- --ignored`\n\
...
fn census_dimensions_match_the_committed_dimension_table() {
    ...
    for (row, &(id, width_bits, height_bits)) in census.iter().zip(&dimensions) {
        ...
        let shipped = lodestone_v770::entity_dimensions::base_dimensions(row.id as i32)
