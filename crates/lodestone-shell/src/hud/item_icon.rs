...
/// Vanilla's pickup-"pop" destination rect, as a pure
...
/// Batched textured faces for the tab-list overlay. Remote sheets are only
/// pulled from the already-decoded cache here; frame gather owns requests, so
/// this renderer never waits on network or image decoding during a draw.
#[derive(Debug)]
pub(crate) struct TabHeadRenderer {
    pipeline: wgpu::RenderPipeline,
    bind_layout: wgpu::BindGroupLayout,
    sampler: wgpu::Sampler,
    textures: HashMap<String, wgpu::BindGroup>,
    defaults: HashMap<String, lodestone_assets::Image>,
...
impl TabHeadRenderer {
    pub(crate) fn new(device: &wgpu::Device, color_format: wgpu::TextureFormat) -> Self {
        ...
        let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("hud-tab-head-pipeline-layout"),
            bind_group_layouts: &[Some(&bind_layout)],
            immediate_size: 0,
        });
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("hud-tab-head-pipeline"),
            layout: Some(&layout),
        ...
        let sampler = device.create_sampler(&wgpu::SamplerDescriptor {
            label: Some("hud-tab-head-sampler"),
            mag_filter: wgpu::FilterMode::Nearest,
            min_filter: wgpu::FilterMode::Nearest,
            ..Default::default()
        });
        let capacity_floats = 256;
        let buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("hud-tab-head-verts"),
            size: (capacity_floats * 4) as wgpu::BufferAddress,
            usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
        Self { pipeline, bind_layout, sampler, textures: HashMap::new(), defaults: HashMap::new(), defaults_loaded: false, buffer, capacity_floats }
    }

    fn ensure_defaults(&mut self, device: &wgpu::Device, queue: &wgpu::Queue) {
        if self.defaults_loaded { return; }
        self.defaults_loaded = true;
        let defaults = crate::resources::load_entity_variant_textures();
        for (key, image) in defaults {
            let view = entity_sheet_texture(device, queue, &image);
            let bind = device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("hud-tab-head-default-bind"),
            ...
            self.textures.insert(format!("default:{key}"), bind);
            self.defaults.insert(key, image);
        }
    }

    fn install_remote(&mut self, device: &wgpu::Device, queue: &wgpu::Queue, url: &str) {
        if self.textures.contains_key(url) { return; }
        let Some(image) = crate::remote_skins::sheet(url) else { return };
        let view = entity_sheet_texture(device, queue, &image);
        let bind = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("hud-tab-head-skin-bind"),
            layout: &self.bind_layout,
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(&view) },
                wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(&self.sampler) },
        ...
        self.textures.insert(url.to_owned(), bind);
    }
    ...
    pub(crate) fn prepare(
        ...
        device: &wgpu::Device,
        queue: &wgpu::Queue,
        heads: &[super::TabHeadDraw],
        viewport_w: f32,
        viewport_h: f32,
    ) -> Vec<TabHeadBatch> {
        if heads.is_empty() { return Vec::new(); }
        self.ensure_defaults(device, queue);
        let mut grouped: Vec<(String, Vec<f32>)> = Vec::new();
        for draw in heads {
            let remote = draw.head.skin_url.as_deref();
            if let Some(url) = remote { self.install_remote(device, queue, url); }
            let key = remote.filter(|u| self.textures.contains_key(*u)).map_or_else(
                || format!("default:{}", draw.head.fallback_sheet), str::to_owned,
            );
            let (image_w, image_h) = if remote.is_some_and(|url| key == url) {
                let Some(image) = crate::remote_skins::sheet(remote.expect("remote is present")) else { continue; };
                (image.width, image.height)
            } else {
                let Some(image) = self.defaults.get(draw.head.fallback_sheet) else { continue; };
                (image.width, image.height)
            };
            let mut verts = Vec::new();
            push_tab_head(&mut verts, viewport_w, viewport_h, draw, image_w, image_h);
            if let Some((_, data)) = grouped.iter_mut().find(|(k, _)| k == &key) {
                data.extend(verts);
            } else {
                grouped.push((key, verts));
            }
        }
        let total: usize = grouped.iter().map(|(_, v)| v.len()).sum();
        if total == 0 { return Vec::new(); }
        if total > self.capacity_floats {
            self.capacity_floats = total.next_power_of_two();
        ...
        for (texture, verts) in grouped {
            let start = (flat.len() / SPRITE_FLOATS_PER_VERTEX) as u32;
            flat.extend(verts);
            let end = (flat.len() / SPRITE_FLOATS_PER_VERTEX) as u32;
            batches.push(TabHeadBatch { texture, start, end });
        }
        queue.write_buffer(&self.buffer, 0, bytemuck::cast_slice(&flat));
        batches
    }

    pub(crate) fn draw(&self, pass: &mut wgpu::RenderPass<'_>, batches: &[TabHeadBatch]) {
        if batches.is_empty() { return; }
        pass.set_pipeline(&self.pipeline);
        pass.set_vertex_buffer(0, self.buffer.slice(..));
        for batch in batches {
            let Some(bind) = self.textures.get(&batch.texture) else { continue; };
...
fn push_tab_head(
    ...
    viewport_w: f32,
    viewport_h: f32,
    draw: &super::TabHeadDraw,
    image_w: u32,
    image_h: u32,
) {
    let to_ndc = |px: f32, py: f32| (2.0 * px / viewport_w - 1.0, 1.0 - 2.0 * py / viewport_h);
    let quad = |verts: &mut Vec<f32>, u0: f32, v0: f32, u1: f32, v1: f32| {
        let (x0, y0) = to_ndc(draw.x, draw.y);
        let (x1, y1) = to_ndc(draw.x + 8.0, draw.y + 8.0);
        let mut v = |x: f32, y: f32, u: f32, vv: f32| verts.extend_from_slice(&[x, y, u, vv, 1.0, 1.0, 1.0, 1.0]);
    ...
    quad(verts, 8.0 / iw, 8.0 / ih, 16.0 / iw, 16.0 / ih);
    if draw.head.show_hat { quad(verts, 40.0 / iw, 8.0 / ih, 48.0 / iw, 16.0 / ih); }
}

#[cfg(test)]
mod tab_head_tests {
    use super::push_tab_head;
    use crate::hud::TabHeadDraw;

    #[test]
    fn face_and_hat_sample_the_two_skin_layers() {
        let draw = TabHeadDraw {
            head: crate::tablist::TabListHead { skin_url: None, fallback_sheet: "entity/player/slim/steve", show_hat: true },
            x: 10.0,
            y: 20.0,
        };
        let mut verts = Vec::new();
        push_tab_head(&mut verts, 100.0, 100.0, &draw, 64, 64);
        assert_eq!(verts.len(), super::SPRITE_FLOATS_PER_VERTEX * 12);
        let uvs: Vec<[f32; 2]> = verts.chunks_exact(super::SPRITE_FLOATS_PER_VERTEX).map(|v| [v[2], v[3]]).collect();
        assert_eq!(uvs[0], [8.0 / 64.0, 8.0 / 64.0]);
        assert_eq!(uvs[6], [40.0 / 64.0, 8.0 / 64.0]);
    }

    #[test]
    fn hidden_hat_emits_only_the_base_face() {
        let draw = TabHeadDraw {
            head: crate::tablist::TabListHead { skin_url: None, fallback_sheet: "entity/player/slim/steve", show_hat: false },
            x: 0.0,
            y: 0.0,
        };
        let mut verts = Vec::new();
        push_tab_head(&mut verts, 100.0, 100.0, &draw, 64, 64);
        assert_eq!(verts.len(), super::SPRITE_FLOATS_PER_VERTEX * 6);
    }
}
