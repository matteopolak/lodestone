...
/// Builds the right-edge sidebar view from the folded game scoreboard.
#[must_use]
pub fn sidebar_from(scoreboard: &Scoreboard) -> Option<Sidebar> {
        ...
        .map(|(holder, entry)| {
            ...
            SidebarLine {
                label: entry
                    ...
                    .to_plain_string(),
                score: score_text(entry.value, number_format),
            }
        })
        .collect();
    Some(Sidebar {
        title: objective.display_name.to_plain_string(),
        lines,
    })
}

fn score_text(value: i32, format: &NumberFormat) -> String {
    match format {
        NumberFormat::Blank => String::new(),
        NumberFormat::Fixed(text) => text.to_plain_string(),
...
mod tests {
    ...
    fn sidebar_uses_game_scoreboard_scores_and_display_names() {
        ...
        let side = sidebar_from(&scoreboard).expect("sidebar visible");
    ...
    fn sidebar_reaches_pixels_inside_right_edge_rect() {
        ...
        scoreboard.set_score("kills", "Bob", 3);
        let sidebar = sidebar_from(&scoreboard).expect("sidebar visible");
