...
//!
//! This table is generated by hand-transcription from the decompiled 26.2 client source's own
//! creative-mode-tabs registration class. Each
//! [`CreativeTab`] mirrors one tab-registration block in its own tab bootstrap:
...
//!
//! - Vanilla's own per-family colored-item registration helper (building_blocks/colored_blocks/
//!   functional_blocks) expands to 16 entries in vanilla's own gameplay-color-order table
//!   (white, light_gray, gray, black, brown, red, orange, yellow, lime, green, cyan,
//!   light_blue, blue, purple, magenta, pink -- read from vanilla's own creative-tab
//!   declarations, and it is
//!   neither alphabetical nor `DyeColor` ordinal order).
//! - Vanilla's own copper-block-family iteration (weathering stages, then waxed
//!   counterparts)
...
//! - Vanilla's own per-family iteration over a weathering-copper item collection
//!   (functional_blocks'
//!   copper lantern/chain/bulb/golem-statue/chest, redstone_blocks' waxed copper bulb, and
//!   the lightning rod) expands to that family's 4 (or, for a bare unweathered-only
//!   iteration, 8: 4
//!   weathering + 4 waxed) items the same way.
//! - Vanilla's own per-family iteration over a dyed-color item collection (the dye,
//!   the harness,
//!   the dyed bundle) expands to 16 entries, but in that collection's own field-
//!   declaration order (white, orange, magenta, light_blue, yellow, lime, pink, gray,
//!   light_gray, cyan, purple, blue, brown, green, red, black -- see its own
//!   constructor and iteration order), which is **not** the gameplay color order. The dye's
//!   own iteration
//!   in the ingredients tab uses this order, not the gameplay color order.
//! - The white wool item accessor (redstone_blocks) -> `minecraft:white_wool`.
//! - The unweathered, unwaxed lightning rod's waxed accessor (redstone_blocks) -> `minecraft:waxed_lightning_rod`.
//! - The unweathered, unwaxed copper chest's waxed accessor (redstone_blocks) -> `minecraft:waxed_copper_chest`.
//!   Both confirmed present in `lodestone-data`'s generated item table (not the unwaxed
//!   fallback the task brief allowed for).
//! - Two Mojang field-name quirks in vanilla's own item-registration declarations, resolved against the generated item table:
//!   the cut-standstone-slab field (sic) -> `minecraft:cut_sandstone_slab`; the dry-short-grass
//!   / dry-tall-grass fields -> `minecraft:short_dry_grass` / `minecraft:tall_dry_grass` (the
...
//! - `op_blocks`' test-block-mode loop (4 modes: start/log/fail/accept, each stamping a
//!   component onto the test-block item) and its light-level loop (16 levels, 15..=0, onto
//!   the light item) are both skipped as component-bearing (see below), but per the task's
//!   instruction the plain base items `minecraft:test_block` and `minecraft:light` are
//!   each emitted once, in the loops' place.
//! - Vanilla's own potion-effect-type generator (combat: the tipped arrow; food_and_drinks:
//!   the potion/splash-potion/lingering-potion items, in that call order) --
//!   one stack per registered `Potion`, in `minecraft:potion`'s own registration order
//!   (`registries.json`'s `protocol_id`, confirmed to match vanilla's own potion
//!   declaration order by a side-by-side read). Vanilla's own max-level-only
//!   enchanted-book generator
...
//!
//! - Vanilla's own ominous-banner-instance generator (functional_blocks) -- an ominous banner stack built
//!   from the banner-pattern registry.
//! - Vanilla's own accept-all-temp-items call (search tab) -- built at runtime from every other tab's
//!   search-visible items; unaffected by anything above since it unions whatever *is* here.
//! - Vanilla's own preset-paintings generator (functional_blocks: placeable variants; op_blocks: the
//!   non-placeable variants under its own has-permissions gate) -- one stack per `PaintingVariant`,
//!   sorted by area then width.
//! - Vanilla's own all-durations firework generator (tools_and_utilities, combat) -- one
//!   `minecraft:firework_rocket` stack per craftable duration (vanilla's own
//!   craftable-durations constant, `{1, 2, 3}`).
//! - Vanilla's own instrument-types generator (tools_and_utilities) -- one `minecraft:goat_horn` stack
//!   per instrument in the `goat_horns` tag.
//! - Vanilla's own suspicious-stews generator (food_and_drinks) -- one `minecraft:suspicious_stew` per
//!   registered suspicious-effect holder.
//! - Vanilla's own ominous-bottles generator (food_and_drinks) -- 5 `minecraft:ominous_bottle` stacks,
//!   amplifier 0..=4.
//! - Vanilla's own all-levels enchanted-book generator (search tab only) -- every level of every
...
//! - **Enchanted-book stacks carry no real `minecraft:stored_enchantments`.**
//!   `minecraft:enchantment` is data-driven, so its network registry id is assigned
//!   per-connection by configuration-phase sync order -- there is no fixed id a static
//!   table could bake in (see [`lodestone_data::enchantment`]'s module doc). What each of
//!   the 43 grid cells *does* carry is an `AuthoredEnchantment` -- the enchantment's name
//!   and (always max) level, both resolvable with no session in hand -- which
//!   `container::tooltip` renders as real lore text, so the tooltip is no longer generic.
//!   The **icon** still matches vanilla's own (identical for every enchantment): drawing a
//!   distinct cover per enchantment would need per-enchantment texture data this build does
...
//! ## How to change it
//!
//! Re-run the transcription against a newer version of vanilla's own creative-tab
//! declarations if vanilla's tab layout
...
//! addition, since vanilla's own item-registration declarations' constant *names* are not always the registry id (see the
...
mod tests {
    ...
    /// by hand against vanilla's own potion declarations' field declaration order.
