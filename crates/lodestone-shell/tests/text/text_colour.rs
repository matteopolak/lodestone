...
//! the code under test: `TextColor`'s own decompiled source in `.cache/mc/26.2/src`, where
...
///
/// Hand-transcribed from `TextColor`'s own decompiled source (26.2). The decimal vanilla
...
fn hex_surfaces_with(coloured: bool) -> Vec<(&'static str, u32, HudGeometry)> {
    ...
    let view = TabListView {
        rows: vec![TabListRow {
            ...
            spectator: false,
            head: lodestone::tablist::TabListHead {
                skin_url: None,
