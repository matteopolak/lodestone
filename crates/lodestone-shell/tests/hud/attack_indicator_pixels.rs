...
//! (`Hud`'s own decompiled source, `.cache/mc/26.2/client-src`) — which `hud.rs`'s
...
//!    (`Hud`'s own decompiled source, `else if (attackStrengthScale < 1.0F)`) hides the
...
fn the_attack_indicator_reaches_the_screen_through_the_real_hud_path() {
    ...
    assert_eq!(
        ...
        "control failed to fail: the indicator drew {full_px} px at full charge (1.0), where \
         vanilla's own rule (`Hud`'s own decompiled source) hides it entirely absent the unimplemented \
         'ready' icon variant"
    );
    assert_eq!(
        none_px, 0,
