...
//! `yLineAir = yLineBase - 10`, `Hud`'s own decompiled source). Nothing else in this
...
//! maxAirSupplyTicks` (`Hud`'s own decompiled source) — an **or**. The first draft of this gate
...
/// (`Entity`'s own decompiled source), the same
...
fn the_air_bubble_row_reaches_the_screen_through_the_real_hud_path() {
    ...
    assert_eq!(
        ...
        "control failed to fail: the row drew {full_dry_px} px on full air out of water, \
         where both of vanilla's disjuncts (`Hud`'s own decompiled source) are false"
    );
    assert!(
        full_wet_px > 200,
