...
//! `v770` — the only family that can host, per `CLAUDE.md`). A malicious or
...
//!
//! `crates/protocol/v770/src/server_protocol.rs` is owned by another agent
...
fn serverbound_entries(state: ConnectionState) -> &'static [(&'static str, i32)] {
    use lodestone_v770::packet_ids::{configuration, handshaking, login, play, status};
    match state {
    ...
    for state in STATES {
        for &(name, id) in serverbound_entries(state) {
            for payload in FIXED_PAYLOADS {
                ...
                assert!(
                    result.is_ok(),
                    "v770 serverbound: {name} (id {id}) panicked on {state:?} payload {payload:02x?}: {}",
                    result.unwrap_err(),
                );
            }
        }
...
proptest! {
    ...
    fn decode_never_panics(
        ...
        prop_assert!(
            result.is_ok(),
            "v770 serverbound: state {:?} packet_id {} payload len {} panicked: {}",
