...
fn crimson_forest_roots_keep_the_source_border_context() {
    ...
    assert_eq!(
        column.block_state_id(14, 43, 14),
...
fn lifecycle_wavefront_keeps_roots_and_clears_prior_fungus_border() {
    ...
    assert_eq!(target.block_state_id(14, 43, 14), Block::CrimsonRoots.default_state());
    assert_eq!(target.block_state_id(2, 51, 1), Block::Air.default_state());
    assert_eq!(target.block_state_id(1, 45, 5), Block::NetherWartBlock.default_state());
}
