...
//!
//! `v770_clientbound_decode.rs` derives `packet_id` from 4 raw fuzzer bytes
...
//! generated table production code and `v770_clientbound_decode.rs` both use),
//! so every declared packet id is one small-integer mutation away rather than
//! a 1-in-4-billion guess. `v770_clientbound_decode.rs` is not redundant with
...
//!
//! Same panic contract as `v770_clientbound_decode.rs`: a panic here is a real
