...
//!
//! The other half of `v26_2_clientbound_decode.rs`'s
//! threat model: this is the attack surface our *integrated server* faces
//! from a connecting client, not what a server can do to us. v26-2 is the only
//! family fuzzed on this side because it is the only one that implements
//! `ServerProtocol` (`CLAUDE.md`: only v26-2 can host). Mirrors
//! `crates/lodestone-fuzz/tests/no_panic_v26_2_serverbound.rs`'s proptest
...
//! `no_panic_v26_2_serverbound.rs`'s own module doc states.
