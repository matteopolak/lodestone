...
pub(crate) use packet_reports::{
    format_rust_source, shape_review_violations,
};
#[cfg(test)]
pub(crate) use packet_reports::{
    first_party_manifest_license_violations, parse_hex_packet_id, resolve_canonical_alias,
};
...
pub fn run_cli_command(command: CliCommand) -> Result<()> {
    match command {
        ...
        CliCommand::CheckWorldgenSchedule => {
            let workspace_root =
                std::env::current_dir().context("determine current workspace directory")?;
            let report = worldgen_schedule::check_worldgen_schedule(&workspace_root)?;
            print!("{}", report.render());
            if report.has_violations() {
...
pub(crate) use packet_ids::{
    load_minecraft_data_protocol_json,
};
...
pub(crate) use connectedness::{
    ClientboundVerdict, ConsumerOutlet, FunctionBody, ServerboundDecodeVerdict,
    classify_clientbound_dispatch,
...
#[cfg(test)]
pub(crate) use deletability::line_forwards_to_family_feature;
